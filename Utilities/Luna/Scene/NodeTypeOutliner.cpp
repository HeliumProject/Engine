...
// Constructor
// 
NodeTypeOutliner::NodeTypeOutliner( Luna::SceneManager* sceneManager )
...
// 
void NodeTypeOutliner::AddNodeType( Luna::SceneNodeType* nodeType )
{
  m_TreeCtrl->Freeze();
  bool isSortingEnabled = m_TreeCtrl->IsSortingEnabled();
  m_TreeCtrl->DisableSorting();
  ...
  for ( ; itr != end; ++itr )
  {
    ...
    if ( dependency->HasType( Reflect::GetType<Luna::SceneNode>() ) )
    {
      AddInstance( Reflect::DangerousCast< Luna::SceneNode >( dependency ) );
    }
  }
...
// that it belongs to.
// 
void NodeTypeOutliner::AddInstance( Luna::SceneNode* instance )
{
...
// 
void NodeTypeOutliner::RemoveNodeType( Luna::SceneNodeType* nodeType )
{
  LUNA_SCENE_SCOPE_TIMER( ("") );
#ifdef _DEBUG
  ...
  if ( found != m_Items.end() )
  {
    ...
    NOC_ASSERT( m_TreeCtrl->GetChildrenCount( item ) == 0 );
  }
#endif
...
// 
void NodeTypeOutliner::RemoveInstance( Luna::SceneNode* instance )
{
  DeleteItem( instance );
}
...
void NodeTypeOutliner::OnBeginLabelEdit( wxTreeEvent& args )
{
  ...
  if ( !found || !found->HasType( Reflect::GetType<Luna::HierarchyNode>() ) )
  {
    args.Veto();
  }
}
...
// 
void NodeTypeOutliner::CurrentSceneChanged( Luna::Scene* oldScene )
{
  AddNodeTypes();
}
