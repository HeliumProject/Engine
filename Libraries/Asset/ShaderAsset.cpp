...
void ShaderAsset::EnumerateClass( Reflect::Compositor<ShaderAsset>& comp )
{
  ...
  bool ShaderAsset::ValidateEngineType( tuid shaderID, EngineTypes::EngineType engineType )
  {
    bool isOk = false;
    ...
    try
    {
      shader = AssetClass::GetAssetClass< ShaderAsset >( shaderID );
    }
    catch ( const Asset::Exception& ex )
    {
      Console::Warning( "ShaderAsset::ValidateEngineType - %s\n", ex.what() );
      isOk = false;
    }

    if ( shader )
    {
      S_EngineType engineTypes;
      shader->GetAllowableEngineTypes( engineTypes );
      isOk = engineTypes.find( engineType ) != engineTypes.end();
    }

    return isOk;
  }

  void ShaderAsset::GetAllowableEngineTypes( S_EngineType& engineTypes ) const
  {
    engineTypes.insert( EngineTypes::Foliage );
    engineTypes.insert( EngineTypes::Moby );
    engineTypes.insert( EngineTypes::Shrub );
  ...
  bool ShaderAsset::ValidateClass( std::string& error ) const
  {
    ...
    else if ( colorMap->m_FileID == TUID::Null )
    {
  ...
  void ShaderAsset::SetTextureDirty( tuid textureID, bool dirty )
  {
    ...
    for ( ; attrItr != attrEnd; ++attrItr )
    {
      ...
      if ( attrib->HasType( Reflect::GetType< TextureMapAttribute >() ) )
      {
        ...
        if ( texAttrib->m_FileID == textureID )
        {
