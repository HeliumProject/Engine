...
u32 igDXRender::Scene::LoadNewMesh( const char* fname, ObjectLoaderPtr loader, int bangleIndex )
{
  ...
  printf("Computing Bounding info\n");
  ...
  {
    printf("Relocating and rescaling\n");
...
u32 igDXRender::Scene::ExtractMesh(const char* name, ObjectLoaderPtr loader, int bangleIndex)
{
  ...
  if ( bangleIndex > 0 )
  {
    ...
    sprintf( meshName, "%s_bangle_%d", name, bangleIndex );
    fname = meshName;
  }
...
/*  char buff[1024];
  ...
  for (u32 f=0;f<frag_count;f++)
  {
    ...
    {
      sprintf( shader_name, "%s.xml", loader->m_fragments[f].m_shader.c_str() );
    }
...
void CreateDefaultMeshes(igDXRender::Render* render)
{
  ...
  g_loaded_meshes.push_back(InitMesh(render,"@@torus_deformed",torus2,40,40));
  g_loaded_meshes.push_back(InitMesh(render,"@@knot",knot,40,40));
  g_loaded_meshes.push_back(InitMesh(render,"@@mobius",mobius,40,40));
  printf("Default meshes created\n");
}
...
void igDXRender::Scene::RemoveAllMeshes()
{
  ...
  for (u32 i=0;i<count;i++)
  {
    ...
    NOC_ASSERT( g_loaded_meshes[i] ? g_loaded_meshes[i]->m_filename.substr( 0, 2 ).compare( "@@" ) == 0 : true );
