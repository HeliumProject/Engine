...
//
//  PickVisitor NOC_ABSTRACT class
//

PickVisitor::PickVisitor(const Luna::Camera* camera)
: m_Flags (0x0)
, m_Camera (camera)
...
LinePickVisitor::LinePickVisitor(const Luna::Camera* camera, const int x, const int y)
: PickVisitor (camera)
{
  camera->ViewportToLine (x, y, m_WorldSpaceLine);
}

LinePickVisitor::LinePickVisitor(const Luna::Camera* camera, const Math::Line& line)
: PickVisitor (camera)
...
FrustumPickVisitor::FrustumPickVisitor(const Luna::Camera* camera, const int pixelX, const int pixelY, const float pixelBoxSize)
: PickVisitor (camera)
{
...
FrustumPickVisitor::FrustumPickVisitor(const Luna::Camera* camera, const Math::Frustum& worldSpaceFrustum)
: PickVisitor (camera)
...
FrustumLinePickVisitor::FrustumLinePickVisitor(const Luna::Camera* camera, const int pixelX, const int pixelY, const float pixelBoxSize)
: PickVisitor(camera)
...
FrustumLinePickVisitor::FrustumLinePickVisitor(const Luna::Camera* camera, const Math::Line& line, const Math::Frustum& worldSpaceFrustum)
: PickVisitor(camera)
...
bool FrustumLinePickVisitor::PickPoint(const Math::Vector3& point, const float err)
{
  ...
  {
    if (!LinePickVisitor::PickPoint(point, Math::BigFloat))
    {
...
bool FrustumLinePickVisitor::PickSegment(const Math::Vector3& p1,const Math::Vector3& p2, const float err)
{
  if (m_PickSpaceFrustum.IntersectsSegment (p1, p2))
  {
    if (!LinePickVisitor::PickSegment(p1, p2, Math::BigFloat))
    {
...
bool FrustumLinePickVisitor::PickTriangle(const Math::Vector3& v0,const Math::Vector3& v1,const Math::Vector3& v2, const float err)
{
  if (m_PickSpaceFrustum.IntersectsTriangle (v0, v1, v2))
  {
    if (!LinePickVisitor::PickTriangle(v0, v1, v2, Math::BigFloat))
    {
      Math::Vector3 point;
...
bool FrustumLinePickVisitor::PickSphere(const Math::Vector3& center, const float radius)
{
  if (m_PickSpaceFrustum.IntersectsPoint(center, radius))
  {
    if (!LinePickVisitor::PickSphere(center, Math::BigFloat))
    {
...
void PickHit::Sort(Luna::Camera* camera, const V_PickHitSmartPtr& hits, V_PickHitSmartPtr& sorted, PickSortType sortType)
{
