...
namespace SQL
{
  // Returns an error string for the given SQLITE_* error code
  SQL_API std::string SQLiteErrCodeToString( int errCode );
  ...
  public:
    ...
    virtual int         ExecSQL( const char* sql ) NOC_OVERRIDE;
    int                 ExecSQL( const char* sql, sqlite3_callback callback, void* callback_arg = NULL );
  ...
  protected:
    ...
    void        SetLastError( const char* function, const char* errMsg = "", const char* msgFormat = "", ... );
