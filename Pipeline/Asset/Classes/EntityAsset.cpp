...
void EntityAsset::EnumerateClass( Reflect::Compositor<EntityAsset>& comp )
{
    ...
    staticTemplate->m_Name = TXT( "Static Entity" );
    staticTemplate->m_Description = TXT( "Static entities are portions of the environment that can be instanced." );
    ...
    dynamicTemplate->m_Name = TXT( "Dynamic Entity" );
    dynamicTemplate->m_Description = TXT( "A dynamic entity is an animated character or physics-related object." );
    ...
    uniqueTemplate->m_Name = TXT( "Unique Geometry" );
    uniqueTemplate->m_Description = TXT( "A piece of geometry that is unique to a particular level." );
...
bool EntityAsset::ValidateCompatible( const Component::ComponentPtr& attr, tstring& error ) const
{
