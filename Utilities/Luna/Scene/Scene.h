...
namespace Luna
{
  ...
  struct NodeChangeArgs
  {
    ...
    NodeChangeArgs( Luna::SceneNode* node )
      : m_Node( node )
    {

    }
  };
  typedef Nocturnal::Signature< void, const NodeChangeArgs& > NodeChangeSignature;

  // node types appear in the objects window in the UI, this event helps keep it up to date
  struct NodeTypeExistenceArgs
  {
    Luna::SceneNodeType* m_NodeType;

    NodeTypeExistenceArgs( Luna::SceneNodeType* nodeType )
      : m_NodeType (nodeType)
    {

    }
  };
  typedef Nocturnal::Signature< void, const NodeTypeExistenceArgs& > NodeTypeExistenceSignature;
  ...
  struct LoadArgs
  {
    ...
    LoadArgs( Luna::Scene* scene, bool loadedOk = false )
      : m_Scene( scene )
      , m_Success( loadedOk )
    {
  ...
  typedef Nocturnal::Signature< void, const LoadArgs& > LoadSignature;
  ...
  struct ExecuteArgs
  {
    ...
    bool m_Interactively;
      ...
      ExecuteArgs( Luna::Scene* scene, bool interactively )
      : m_Scene( scene )
  ...
  typedef Nocturnal::Signature< void, const ExecuteArgs& > ExecuteSignature;
  ...
  class Transform;
  typedef std::map< UniqueID::TUID, const Luna::Transform* > M_TransformConstDumbPtr;
  ...
  private:
    ...
    std::set<UniqueID::TUID> m_LastHidden;
  ...
  public:
    ...
    Luna::SceneManager* GetManager() const
    {
      return m_Manager;
    }
    ...
    Selection& GetSelection()
    {
      ...
      NOC_ASSERT( !m_UndoQueue.IsActive() );
    ...
    const Luna::TransformPtr& GetRoot() const
    {
      return m_Root;
    }
    ...
    Luna::SceneNode* Get( const UniqueID::TUID& uid ) const
    {
    ...
    template< class T >
    T* Get( const UniqueID::TUID &uid ) const
    {
  ...
  private:
    ...
    void ArchiveStatus(Reflect::StatusInfo& info)NOC_OVERRIDE;
    void ArchiveException(Reflect::ExceptionInfo& info) NOC_OVERRIDE;
    ...
    /// @return returns the id of the source node if it exists. If not it returns UniqueID::TUID::Null
    UniqueID::TUID  GetRemappedID( tuid nodeId );
  ...
  private:
    // saving helpers
    void ExportSceneNode( Luna::SceneNode* node, Reflect::V_Element& elements, UniqueID::S_TUID& exported, const ExportArgs& args, Undo::BatchCommand* changes );
    void ExportHierarchyNode( Luna::HierarchyNode* node, Reflect::V_Element& elements, UniqueID::S_TUID& exported, const ExportArgs& args, Undo::BatchCommand* changes, bool exportChildren = true );


    //
    // Naming
    //  Each object in Luna has a unique name, these interfaces ensure that
  ...
  public:
    ...
    Luna::SceneNode* FindNode( const UniqueID::TUID& id );
    ...
    Luna::HierarchyNode* GetCommonParent( const V_HierarchyNodeDumbPtr& nodes );
  ...
  {
  public:
    SceneNodeExistenceCommand( Undo::ExistenceAction action, Luna::Scene* scene, const SceneNodePtr& node, bool redo = true )
      : Undo::ExistenceCommand( action, new Undo::MemberFunctionConstRef< Luna::Scene, SceneNodePtr >( scene, node, &Luna::Scene::AddObject ), new Undo::MemberFunctionConstRef< Luna::Scene, SceneNodePtr >( scene, node, &Luna::Scene::RemoveObject ), redo )
    {
    }
  };
  ...
  public:
    ...
    SceneSelectCommand( Luna::Scene* scene, OS_SelectableDumbPtr& selection  ) 
      : m_Scene( scene )
