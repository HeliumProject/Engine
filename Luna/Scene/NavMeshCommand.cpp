...
// Constructor 
// 
AddNavMeshCommand::AddNavMeshCommand( Luna::NavMeshPtr mesh, const Math::V_Vector3& positions, u32 mode )
: m_Mesh( mesh )
...
AddNavMeshCommand::AddNavMeshCommand( Luna::NavMeshPtr mesh, const Math::Matrix4& cube_transform,  const Math::Matrix4& cube_inv_transform, u32 mode )
: m_Mesh( mesh )
, m_Mode( mode )
, m_Merged( false )
{
...
void AddNavMeshCommand::Undo()
{
  ...
  if ( !m_Mesh->GetScene()->GetManager()->GetCurrentScene()->GetTool() || m_Mesh->GetScene()->GetManager()->GetCurrentScene()->GetTool()->GetType() != Reflect::GetType<Luna::NavMeshCreateTool>() )
  {
    m_Mesh->ClearEditingState();
  }
}
