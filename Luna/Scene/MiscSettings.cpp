...
static bool ColorModeSort( const ScaleColorModeValue* left, const ScaleColorModeValue* right )
{
...
{
    comp.AddField( &ScaleColorModeValue::m_Percentage, "Percentage"  );
    comp.AddField( &ScaleColorModeValue::m_Red, "Red" );
    comp.AddField( &ScaleColorModeValue::m_Green, "Green" );
    comp.AddField( &ScaleColorModeValue::m_Blue, "Blue" );
}

void MiscSettings::EnumerateClass( Reflect::Compositor<MiscSettings>& comp )
{
    comp.AddField( &MiscSettings::m_ColorModeSettings, "Scale Color Mode Settings" );
}

MiscSettingsPtr MiscSettings::LoadFromFile()
{
#pragma TODO("reimplement to use a valid config folder" )
    tstring miscSettingsFile = TXT( "" );

    MiscSettingsPtr miscSettings;

    if ( Nocturnal::Path( miscSettingsFile ).Exists() )
    {
        try
        {
            miscSettings = Reflect::Archive::FromFile<MiscSettings>( miscSettingsFile );
        }
        catch ( ... )
        {
        }
