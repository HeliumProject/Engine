...
// Constructor
// 
HierarchyOutliner::HierarchyOutliner( Luna::SceneManager* sceneManager )
...
HierarchyOutlinerItemData* HierarchyOutliner::GetTreeItemData( const wxTreeItemId& item )
{
  ...
  NOC_ASSERT( m_TreeCtrl->GetItemData( item ) );
...
// items in this tree.
// 
void HierarchyOutliner::RecurseAddHierarchyNode( Luna::HierarchyNode* node )
{
  LUNA_SCENE_SCOPE_TIMER( ("") );
...
// 
void HierarchyOutliner::AddHierarchyNode( Luna::HierarchyNode* node )
{
  LUNA_SCENE_SCOPE_TIMER( ("") );
...
// 
void HierarchyOutliner::CurrentSceneChanged( Luna::Scene* oldScene )
{
  AddHierarchyNodes();
}
...
///////////////////////////////////////////////////////////////////////////////
// Since we NOC_OVERRIDE ConnectSceneListeners, we also NOC_OVERRIDE this function to
...
void HierarchyOutliner::OnEndDrag( wxTreeEvent& args )
{
  ...
  if ( dropItem.IsOk() && !m_TreeCtrl->IsSelected( dropItem ) )
  {
    ...
    Luna::HierarchyNode* newParent = GetTreeItemData( dropItem )->GetHierarchyNode();
    NOC_ASSERT( newParent );
    ...
    {
      Luna::HierarchyNode* hNode = Reflect::ObjectCast< Luna::HierarchyNode >( *selItr );
      if ( hNode )
      {
...
void HierarchyOutliner::ParentChanged( const ParentChangedArgs& args )
{
  ...
  Luna::HierarchyNode* child = args.m_Node;
  m_TreeCtrl->Freeze();
...
void HierarchyOutliner::NodeAdded( const NodeChangeArgs& args )
{
  ...
  if ( args.m_Node->HasType( Reflect::GetType<Luna::HierarchyNode>() ) )
  {
    m_TreeCtrl->Freeze();
    bool isSortingEnabled = m_TreeCtrl->IsSortingEnabled();
    m_TreeCtrl->DisableSorting();

    Luna::HierarchyNode* hierarchyNode = Reflect::DangerousCast< Luna::HierarchyNode >( args.m_Node );
