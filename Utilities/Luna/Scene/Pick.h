...
namespace Luna
{
  ...
  public: 
    PickVisitor(const Luna::Camera* camera);
  ...
  public:
    ...
    {
      NOC_ASSERT( count <= m_PickHits.size() );
  ...
  public:
    LinePickVisitor(const Luna::Camera* camera, const int x, const int y);
    LinePickVisitor(const Luna::Camera* camera, const Math::Line& line);

  protected:
    LinePickVisitor(const LinePickVisitor& rhs) : PickVisitor (rhs) { }

  public:
    virtual const PickTypes::PickType GetPickType() const NOC_OVERRIDE
    {
    ...
    virtual void Transform() NOC_OVERRIDE;
    
    // picking functions (produce hits)
    virtual bool PickPoint(const Math::Vector3& p, const float err = Math::LinearIntersectionError) NOC_OVERRIDE;
    virtual bool PickSegment(const Math::Vector3& p1,const Math::Vector3& p2, const float err = Math::LinearIntersectionError) NOC_OVERRIDE;
    virtual bool PickTriangle(const Math::Vector3& v0,const Math::Vector3& v1,const Math::Vector3& v2, const float err = Math::LinearIntersectionError) NOC_OVERRIDE;
    virtual bool PickSphere(const Math::Vector3& center, const float radius) NOC_OVERRIDE;
    virtual bool PickBox(const Math::AlignedBox& box) NOC_OVERRIDE;

    // testing functions (no hits)
    virtual bool IntersectsBox(const Math::AlignedBox& box) const NOC_OVERRIDE;
  ...
  public:
    FrustumPickVisitor(const Luna::Camera* camera, const int pixelX, const int pixelY, const float pixelBoxSize = -1.0f);
    FrustumPickVisitor(const Luna::Camera* camera, const Math::Frustum& worldSpaceFrustum);
  ...
  {
  public:
    FrustumLinePickVisitor(const Luna::Camera* camera, const int pixelX, const int pixelY, const float pixelBoxSize = -1.0f);
    FrustumLinePickVisitor(const Luna::Camera* camera, const Math::Line& line, const Math::Frustum& worldSpaceFrustum); 
  ...
  class PickHit : public Nocturnal::RefCountBase<PickHit>
  {
    //
    // Object hit
    //
  ...
  public:
    ...
    static void Sort(Luna::Camera* camera, const V_PickHitSmartPtr& hits, V_PickHitSmartPtr& sorted, PickSortType sortType);
  };
}
