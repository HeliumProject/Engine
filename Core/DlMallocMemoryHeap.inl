...
/// Wrapper for Lunar::PhysicalMemory::Allocate() to support return values expected by dlmalloc.
///
...
{
    void* pMemory = Lunar::PhysicalMemory::Allocate( size );
    return( pMemory ? pMemory : reinterpret_cast< void* >( ~static_cast< uintptr_t >( 0 ) ) );
}

/// Wrapper for Lunar::PhysicalMemory::Free() to support return values expected by dlmalloc.
///
...
{
    return( Lunar::PhysicalMemory::Free( pMemory, size ) ? 0 : -1 );
}
...
#ifdef L_ENABLE_MEMORY_TRACKING_VERBOSE
...
namespace Lunar
{
    ...
    /// @param[in] capacity  Fixed size (in bytes) of the memory heap to create, or zero to create a growable heap.
    MEMORY_HEAP_CLASS_NAME::MEMORY_HEAP_CLASS_NAME( const tchar_t* pName, size_t capacity )
        : m_pName( pName )
#if L_ENABLE_MEMORY_TRACKING_VERBOSE
        , m_pVerboseTrackingData( NULL )
#endif
    ...
    /// Destructor.
    MEMORY_HEAP_CLASS_NAME::~MEMORY_HEAP_CLASS_NAME()
    {
#if L_ENABLE_MEMORY_TRACKING_VERBOSE
    ...
    ///
    /// @see Reallocate(), Free()
    void* MEMORY_HEAP_CLASS_NAME::Allocate( size_t size )
    {
#if L_ENABLE_MEMORY_TRACKING_VERBOSE
        bool bLockedTracking = ConditionalVerboseTrackingLock();
#endif
    ...
    ///
    /// @see Allocate(), Free()
    void* MEMORY_HEAP_CLASS_NAME::Reallocate( void* pMemory, size_t size )
    {
#if L_ENABLE_MEMORY_TRACKING_VERBOSE
    ...
    ///
    /// @see Free()
    void* MEMORY_HEAP_CLASS_NAME::AllocateAligned( size_t alignment, size_t size )
    {
#if L_ENABLE_MEMORY_TRACKING_VERBOSE
        bool bLockedTracking = ConditionalVerboseTrackingLock();
#endif
    ...
    void MEMORY_HEAP_CLASS_NAME::Free( void* pMemory )
    {
#if L_ENABLE_MEMORY_TRACKING_VERBOSE
        bool bLockedTracking = false;
#endif
    ...
    size_t MEMORY_HEAP_CLASS_NAME::GetMemorySize( void* pMemory )
    {
#if USE_NEDMALLOC
    ...
    void MEMORY_HEAP_CLASS_NAME::UnregisterCurrentThreadCache()
    {
    ...
    ///
    /// @param[in] capacity  Fixed size (in bytes) of the memory heap to create, or zero to create a growable heap.
    void MEMORY_HEAP_CLASS_NAME::ConstructNoName( size_t capacity )
    {
#if USE_NEDMALLOC
    ...
    void MEMORY_HEAP_CLASS_NAME::AddAllocation( void* pMemory )
    {
        if( pMemory )
        {
#if USE_NEDMALLOC
    ...
    ///
    /// @see AddAllocation()
    void MEMORY_HEAP_CLASS_NAME::RemoveAllocation( void* pMemory )
    {
        if( pMemory )
        {
#if USE_NEDMALLOC
