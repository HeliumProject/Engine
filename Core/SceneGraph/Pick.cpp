...
bool LinePickVisitor::PickTriangle(const Math::Vector3& v0, const Math::Vector3& v1, const Math::Vector3& v2, const float err)
{
  ...
  float32_t distance = (float32_t)Math::BigFloat;
...
bool LinePickVisitor::AddHitSegment(const Math::Vector3& p1,const Math::Vector3& p2, float32_t mu, Math::Vector3& offset)
{
  // allocate a hit
  PickHit* hit = AddHit ();
...
bool LinePickVisitor::AddHitTriangle(const Math::Vector3& v0,const Math::Vector3& v1,const Math::Vector3& v2, float32_t u, float32_t v, bool interior, Math::Vector3& vertex, Math::Vector3& intersection, float distance)
{
  float32_t dot = 0.f;
...
bool FrustumLinePickVisitor::PickPoint(const Math::Vector3& point, const float err)
{
  ...
  {
    if (!LinePickVisitor::PickPoint(point, (float32_t)Math::BigFloat))
    {
...
bool FrustumLinePickVisitor::PickSegment(const Math::Vector3& p1,const Math::Vector3& p2, const float err)
{
  if (m_PickSpaceFrustum.IntersectsSegment (p1, p2))
  {
    if (!LinePickVisitor::PickSegment(p1, p2, (float32_t)Math::BigFloat))
    {
...
float32_t GetClosestPointOnEdge(const Math::Vector3& edge_start, const Math::Vector3& edge_end, const Math::Vector3& pt, Math::Vector3& closest_pt)
{
...
bool FrustumLinePickVisitor::PickTriangle(const Math::Vector3& v0,const Math::Vector3& v1,const Math::Vector3& v2, const float err)
{
  if (m_PickSpaceFrustum.IntersectsTriangle (v0, v1, v2))
  {
    if (!LinePickVisitor::PickTriangle(v0, v1, v2, (float32_t)Math::BigFloat))
    {
      Math::Vector3 point;
...
bool FrustumLinePickVisitor::PickSphere(const Math::Vector3& center, const float radius)
{
  if (m_PickSpaceFrustum.IntersectsPoint(center, radius))
  {
    if (!LinePickVisitor::PickSphere(center, (float32_t)Math::BigFloat))
    {
