...
EVT_MENU( VaultMenu::Settings, VaultFrame::OnSettings )
...
{
#pragma TODO( "Populate the CollectionsPanel from VaultSettings" )

    HELIUM_ASSERT( m_Vault );
...
#pragma TODO( "pass in the right root directory to the ResultsPanel" )
    ...
    //
    {
        ...
        tstring defaultPath = wxGetApp().GetSettingsManager()->GetSettings< VaultSettings >()->GetDefaultFolderPath();
...
#pragma TODO( "Toggle the Folders and Collections, etc... panels' states in panelsMenu based on loaded preferences." )
        ...
        m_OptionsMenu->AppendSeparator();
        m_OptionsMenu->Append( VaultMenu::Settings, VaultMenu::Label( VaultMenu::Settings ) );
    ...
    // Load Settings
    //
    VaultSettings* vaultSettings = wxGetApp().GetSettingsManager()->GetSettings< VaultSettings >();

    vaultSettings->GetWindowSettings( this, &m_FrameManager );
    vaultSettings->AddChangedListener( Reflect::ElementChangeSignature::Delegate( this, &VaultFrame::OnSettingsChanged ) );
    m_CurrentViewOption = vaultSettings->GetThumbnailMode();
    UpdateResultsView( vaultSettings->GetThumbnailSize() );
    ...
    // Connect Listeners
    m_VaultSearch->AddRequestSearchListener( Editor::RequestSearchSignature::Delegate( this, &VaultFrame::OnRequestSearch ) );
    m_VaultSearch->AddBeginSearchingListener( Editor::BeginSearchingSignature::Delegate( this, &VaultFrame::OnBeginSearching ) );
    m_VaultSearch->AddResultsAvailableListener( Editor::ResultsAvailableSignature::Delegate( this, &VaultFrame::OnResultsAvailable ) );
    m_VaultSearch->AddSearchCompleteListener( Editor::SearchCompleteSignature::Delegate( this, &VaultFrame::OnSearchComplete ) );
...
VaultFrame::~VaultFrame()
{
    ...
    m_VaultSearch->RemoveRequestSearchListener( Editor::RequestSearchSignature::Delegate( this, &VaultFrame::OnRequestSearch ) );
    m_VaultSearch->RemoveBeginSearchingListener( Editor::BeginSearchingSignature::Delegate( this, &VaultFrame::OnBeginSearching ) );
    m_VaultSearch->RemoveResultsAvailableListener( Editor::ResultsAvailableSignature::Delegate( this, &VaultFrame::OnResultsAvailable ) );
...
void VaultFrame::GetSelectedPaths( std::set< Helium::Path >& paths )
{
...
void VaultFrame::OnNewCollectionFromSelection( wxCommandEvent& event )
{
    ...
    if ( collection )
    {
        CollectionManager* collectionManager = wxGetApp().GetSettingsManager()->GetSettings< VaultSettings >()->GetCollectionManager();
        tstring name;
...
///////////////////////////////////////////////////////////////////////////////
void VaultFrame::OnSettings( wxCommandEvent& event )
{
...
///////////////////////////////////////////////////////////////////////////////
void VaultFrame::OnUpdateStatusBar( Editor::UpdateStatusEvent& event )
{
...
void VaultFrame::OnClose( wxCloseEvent& event )
{
    ...
    VaultSettings* vaultSettings = wxGetApp().GetSettingsManager()->GetSettings< VaultSettings >();
    vaultSettings->RemoveChangedListener( Reflect::ElementChangeSignature::Delegate( this, &VaultFrame::OnSettingsChanged ) );

    vaultSettings->SetThumbnailMode( m_CurrentViewOption );
    vaultSettings->SetWindowSettings( this, &m_FrameManager );

    tstring path;
    m_FoldersPanel->GetPath( path );
    vaultSettings->SetDefaultFolderPath( path );
...
// 
void VaultFrame::OnRequestSearch( const Editor::RequestSearchArgs& args )
{
...
// 
void VaultFrame::OnBeginSearching( const Editor::BeginSearchingArgs& args )
{
    m_IsSearching = true;
    m_ResultsPanel->ClearResults();
}
...
// 
void VaultFrame::OnResultsAvailable( const Editor::ResultsAvailableArgs& args )
{
...
// 
void VaultFrame::OnSearchComplete( const Editor::SearchCompleteArgs& args )
{
    m_IsSearching = false;
...
// 
void VaultFrame::OnSettingsChanged( const Reflect::ElementChangeArgs& args )
{
    m_CurrentViewOption = wxGetApp().GetSettingsManager()->GetSettings< VaultSettings >()->GetThumbnailMode();
    UpdateResultsView( wxGetApp().GetSettingsManager()->GetSettings< VaultSettings >()->GetThumbnailSize() );
}
...
void VaultFrame::UpdateNavBar( const SearchQueryPtr& searchQuery )
{
    ...
    {
        Helium::Path path( searchQuery->GetQueryString() );
