...
#include "Editor/Scene/CameraMovedCommand.h"
#include "Editor/Scene/Color.h"
#include "Editor/Scene/Pick.h"
#include "Editor/Scene/PrimitiveAxes.h"
#include "Editor/Scene/PrimitiveFrame.h"
#include "Editor/Scene/PrimitiveGrid.h"
#include "Editor/Scene/PrimitiveRings.h"
#include "Editor/Scene/Resource.h"
#include "Editor/Scene/SceneInit.h"
#include "Editor/Scene/ScenePreferences.h"
#include "Editor/Scene/Statistics.h"
#include "Editor/Scene/Orientation.h"
#include "Editor/Scene/Tool.h"
#include "Editor/UI/EditorIDs.h"
...
void Viewport::InitializeType()
{
    ...
    s_LiveMaterial.Ambient = Editor::Color::MAGENTA;
    s_LiveMaterial.Diffuse = Editor::Color::BLACK;
    s_LiveMaterial.Specular = Editor::Color::BLACK;

    ZeroMemory( &s_SelectedMaterial, sizeof( s_SelectedMaterial ) );
    s_SelectedMaterial.Ambient = Editor::Color::SPRINGGREEN;
    s_SelectedMaterial.Diffuse = Editor::Color::BLACK;
    s_SelectedMaterial.Specular = Editor::Color::BLACK;

    ZeroMemory( &s_RedMaterial, sizeof( s_RedMaterial ) );
    s_RedMaterial.Ambient = Editor::Color::RED;
    s_RedMaterial.Diffuse = Editor::Color::BLACK;
    s_RedMaterial.Specular = Editor::Color::BLACK;

    ZeroMemory( &s_YellowMaterial, sizeof( s_YellowMaterial ) );
    s_YellowMaterial.Ambient = Editor::Color::YELLOW;
    s_YellowMaterial.Diffuse = Editor::Color::BLACK;
    s_YellowMaterial.Specular = Editor::Color::BLACK;

    ZeroMemory( &s_GreenMaterial, sizeof( s_GreenMaterial ) );
    s_GreenMaterial.Ambient = Editor::Color::GREEN;
    s_GreenMaterial.Diffuse = Editor::Color::BLACK;
    s_GreenMaterial.Specular = Editor::Color::BLACK;


    ZeroMemory( &s_ReactiveMaterial, sizeof( s_ReactiveMaterial ) );
    s_ReactiveMaterial.Ambient = Editor::Color::WHITE;
    s_ReactiveMaterial.Diffuse = Editor::Color::BLACK;
    s_ReactiveMaterial.Specular = Editor::Color::BLACK;

    ZeroMemory( &s_HighlightedMaterial, sizeof( s_HighlightedMaterial ) );
    s_HighlightedMaterial.Ambient = Editor::Color::CYAN;
    s_HighlightedMaterial.Diffuse = Editor::Color::BLACK;
    s_HighlightedMaterial.Specular = Editor::Color::BLACK;

    ZeroMemory( &s_UnselectableMaterial, sizeof( s_UnselectableMaterial ) );
    s_UnselectableMaterial.Ambient = Editor::Color::GRAY;
    s_UnselectableMaterial.Diffuse = Editor::Color::BLACK;
    s_UnselectableMaterial.Specular = Editor::Color::BLACK;

    ZeroMemory( &s_ComponentMaterial, sizeof( s_ComponentMaterial ) );
    s_ComponentMaterial.Ambient = Editor::Color::MAGENTA;
    s_ComponentMaterial.Diffuse = Editor::Color::BLACK;
    s_ComponentMaterial.Specular = Editor::Color::BLACK;

    ZeroMemory( &s_SelectedComponentMaterial, sizeof( s_SelectedComponentMaterial ) );
    s_SelectedComponentMaterial.Ambient = Editor::Color::YELLOW;
    s_SelectedComponentMaterial.Diffuse = Editor::Color::BLACK;
    s_SelectedComponentMaterial.Specular = Editor::Color::BLACK;

    ZeroMemory( &s_BlueMaterial, sizeof( s_BlueMaterial ) );
    s_BlueMaterial.Ambient = Editor::Color::BLUE;
    s_BlueMaterial.Diffuse = Editor::Color::BLACK;
    s_BlueMaterial.Specular = Editor::Color::BLACK;
}
...
Editor::Camera* Viewport::GetCamera()
{
    return &m_Cameras[m_CameraMode];
}

const Editor::Camera* Viewport::GetCamera() const
{
    return &m_Cameras[m_CameraMode];
}

Editor::Camera* Viewport::GetCameraForMode(CameraMode mode)
{
...
void Viewport::InitWidgets()
{
    ...
    m_GlobalPrimitives[GlobalPrimitives::ViewportAxes] = new Editor::PrimitiveAxes (m_ResourceTracker);
    m_GlobalPrimitives[GlobalPrimitives::ViewportAxes]->Update();

    m_GlobalPrimitives[GlobalPrimitives::StandardAxes] = new Editor::PrimitiveAxes (m_ResourceTracker);
    m_GlobalPrimitives[GlobalPrimitives::StandardAxes]->Update();

    m_GlobalPrimitives[GlobalPrimitives::StandardGrid] = new Editor::PrimitiveGrid (m_ResourceTracker);
    ...
    m_GlobalPrimitives[GlobalPrimitives::StandardRings] = new Editor::PrimitiveRings (m_ResourceTracker);
    m_GlobalPrimitives[GlobalPrimitives::StandardRings]->Update();

    m_GlobalPrimitives[GlobalPrimitives::TransformAxes] = new Editor::PrimitiveAxes (m_ResourceTracker);
    m_GlobalPrimitives[GlobalPrimitives::TransformAxes]->Update();

    m_GlobalPrimitives[GlobalPrimitives::SelectedAxes] = new Editor::PrimitiveAxes (m_ResourceTracker);
    m_GlobalPrimitives[GlobalPrimitives::SelectedAxes]->Update();

    m_GlobalPrimitives[GlobalPrimitives::JointAxes] = new Editor::PrimitiveAxes (m_ResourceTracker);
    m_GlobalPrimitives[GlobalPrimitives::JointAxes]->Update();

    m_GlobalPrimitives[GlobalPrimitives::JointRings] = new Editor::PrimitiveRings (m_ResourceTracker);
    m_GlobalPrimitives[GlobalPrimitives::JointRings]->Update();

    m_SelectionFrame = new Editor::PrimitiveFrame ( m_ResourceTracker );
...
#pragma TODO("Freeze here -Geoff")
    ...
    else
    {
        ...
        else if (m_Tool) // else if we have a tool object
        {
            ...
            if (m_Tool->MouseDown( input ) || e.MiddleDown() || e.RightDown())
            {
