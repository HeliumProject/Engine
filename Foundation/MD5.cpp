...
#ifdef BYTE_ORDER
#error BYTE_ORDER is defined, feel free to modify this source file
#endif
...
tstring Helium::MD5(const void* data, uint32_t count)
{
    md5_state_t state;
    md5_init(&state);

    md5_append(&state, (const md5_byte_t *)data, count);
...
tstring Helium::MD5(const tstring& data)
{
...
tstring Helium::FileMD5(const tstring& filePath, uint32_t packetSize)
{
    ...
    uint8_t* data = (uint8_t*)alloca( packetSize );
    while ( size )
    {
		size_t read = f.Read(data, packetSize);
        md5_append(&state, data, (int)read);
        size -= read;
    }
	f.Close();
