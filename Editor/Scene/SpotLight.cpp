...
{
  Reflect::RegisterClass< SpotLight >( TXT( "Editor::SpotLight" ) );
...
SpotLight::SpotLight(Editor::Scene* scene)
: Editor::Light (scene, new Content::SpotLight() )
, m_InnerCone (NULL)
, m_OuterCone (NULL)
, m_FirstEntry( true )
{
...
SpotLight::SpotLight(Editor::Scene* scene, Content::SpotLight* light)
...
{
  const Editor::SpotLight*    lSpotLight  = Reflect::ConstAssertCast<Editor::SpotLight>( object );
...
void SpotLight::DrawInner( IDirect3DDevice9* device, DrawArgs* args, const SceneNode* object )
{
  ...
  const Editor::InstanceType* type = Reflect::ConstAssertCast<Editor::InstanceType>( spotLight->GetNodeType() );
