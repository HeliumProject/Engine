...
// 
bool DocumentManager::IsUpToDate( const Document* document ) const
{
...
// 
// 
bool DocumentManager::ValidateDocument( const Document* document, std::string& error ) const
{
...
DocumentPtr DocumentManager::OpenPath( const std::string& path, std::string& error )
{
  ...
  File::GlobalManager().Open( document->GetFilePath() );
...
///////////////////////////////////////////////////////////////////////////////
// Returns true if it successfully opens a document with the specified tuid.
// 
DocumentPtr DocumentManager::OpenFileID( const tuid id, std::string& error )
{
  std::string path = File::GlobalManager().GetPath( id );
  if ( path.empty() )
  {
    std::stringstream msg;
    msg << "Could not resolve file ID [" << TUID::HexFormat << id << "] to a valid file path.";
    error = msg.str();
    return false;
  }

  return OpenPath( path, error );
}
...
{
  // If file is not in the manager, add it
  tuid fileID = TUID::Null;
  try
  {
    fileID = File::GlobalManager().GetID( document->GetFilePath() );
  }
  ...
  if ( fileID == TUID::Null )
  {
    try 
    {
      fileID = File::GlobalManager().Add( document->GetFilePath() );
    }
    catch ( const Nocturnal::Exception& e )
    {
      std::stringstream str;
      str << "Could not add '" << document->GetFilePath() << "' to the file manager.  However, your data was still saved.  Make sure that you saved this file to the right location and contact a Tools Programmer for additional help. ";
      str << "\nReason: " << e.Get();
      wxMessageBox( str.str().c_str(), "Error", wxCENTER | wxICON_WARNING | wxOK, m_ParentWindow );
    }
    // TODO: If auto-submit is not disabled, check in the file and check it back out
  }
...
// 
bool DocumentManager::IsCheckedOut( const Document* document ) const
{
...
// out the file.
// 
bool DocumentManager::CheckOut( const Document* document ) const
{
  if ( !RCS::PathIsManaged( document->GetFilePath() ) )
  {
    return true;
...
// 
bool DocumentManager::QueryCheckOut( const Document* document ) const
{
  if ( !IsUpToDate( document ) )
  {
    std::ostringstream str;
...
// (not the checkout status).
// 
SaveAction DocumentManager::QuerySave( const Document* document ) const
{
...
// 
SaveAction DocumentManager::QueryClose( const Document* document ) const
{
  if ( !document->IsModified() )
  {
    return SaveActions::Skip;
...
// 
SaveAction DocumentManager::QueryCloseAll( const Document* document ) const
{
  if ( document->IsModified() )
  {
    std::ostringstream msg;
