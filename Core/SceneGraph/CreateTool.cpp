...
void CreateTool::CalculateInstanceRadiusAndBounds( float32_t& instanceRadius, Math::AlignedBox& bounds )
{
    SceneGraph::HierarchyNode* node = Reflect::ObjectCast<SceneGraph::HierarchyNode>( m_Instance );
...
void CreateTool::AddToScene()
{
    ...
    {
        CORE_SCOPE_TIMER( ("Remove Transient Instance") );
    ...
    {
        CORE_SCOPE_TIMER( ("Push Undo Command Adding Instance Into Batch") );
    ...
    {
        CORE_SCOPE_TIMER( ("Append To Selection") );

        m_Selection.Append( m_Instance );
    }

    {
        CORE_SCOPE_TIMER( ("Push Undo Batch Into Scene") );
...
bool CreateTool::MouseDown( const MouseButtonInput& e )
{
    if ( e.MiddleDown() )
    {
        ...
        {
            CORE_SCOPE_TIMER( ( "Execute Scene" ) );
...
void CreateTool::CreateSingleObject( const Math::Vector3& translation, const Math::Vector3& normal, bool checkValid )
{
    ...
    {
        CORE_SCOPE_TIMER( ( "Place Temporary Instance At Location" ) );
        Place( orientation );
    }

    {
        CORE_SCOPE_TIMER( ( "Add Instance To Scene" ) );
        AddToScene();
    }
...
{
    CORE_SCOPE_TIMER( ("Place Multiple Instances At Location") );
