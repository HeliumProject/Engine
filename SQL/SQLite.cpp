...
static const char* s_Sqlite3MsgFormat = "sqlite3 [%s %d] -- %s";
...
, m_IsDBOpen( false )
{
  Console::Debug( "SQLite ThreadSafe: %d\n", sqlite3_threadsafe() );
}
  ...
  if ( sqlFile.empty() || dbFile.empty() )
{
  ...
  if ( FileSystem::Exists( dbFile ) )
  {
    if ( failIfExists )
    {
      error = std::string( "Could create DB, DB file already exists: " ) +  dbFile ;
      return result;
    }
  ...
  result = ( Windows::Execute( sqliteCmd, true, true ) == 0 );
...
{ 
  ...
  std::string tempBat;
  Nocturnal::GetEnvVar("TMP", tempBat);
  ...
  result = ( Windows::Execute( tempBat, true, true ) == 0 );
...
//
int SQLite::ExecSQL( const char* sql )
{
...
{
  ThrowIfMaxOpenTrans( "SQLite::BeginTrans" );
...
{
  ThrowIfNoTransOpen( "SQLite::CommitTrans" );
...
{
  ThrowIfNoTransOpen( "SQLite::RollbackTrans" );
...
/////////////////////////////////////////////////////////////////////////////
// Sets m_LastErrMsg with the pretty prints of the last SQLite error message,
...
// 
// Example:
//    sqlite3 [SQLITE_MISUSE 21] -- library routine called out of sequence in ResetStatement;
//    Failed to execute SQL statement: ROLLBACK TRANSACTION;
//
void SQLite::SetLastError( const char* function, const char* errMsg, const char* msgFormat, ... )
{
  //////////////////////////////////////
  // Get the error code
  //
  m_LastErrCode = sqlite3_errcode( m_DB );
  ...
  if ( strcmp( errMsg, "" ) != 0 )
  {
    _snprintf( msgBuffer, sizeof(msgBuffer), s_Sqlite3MsgFormat, SQLiteErrCodeToString( m_LastErrCode ).c_str(), m_LastErrCode, errMsg ); 
  }
  else
  {
    const char* dbErrMsg = sqlite3_errmsg( m_DB );
    _snprintf( msgBuffer, sizeof(msgBuffer), s_Sqlite3MsgFormat, SQLiteErrCodeToString( m_LastErrCode ).c_str(), m_LastErrCode, dbErrMsg );
  }
...
/////////////////////////////////////////////////////////////////////////////
std::string SQL::SQLiteErrCodeToString( int errCode )
{
