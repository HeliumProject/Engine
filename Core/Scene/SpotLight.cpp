...
// RTTI
SCENE_DEFINE_TYPE(SpotLight);

void SpotLight::InitializeType()
{
    Reflect::RegisterClassType< SpotLight >( TXT( "Core::SpotLight" ) );
...
{
    Reflect::UnregisterClassType< SpotLight >();
}

SpotLight::SpotLight(Core::Scene* scene)
: Core::Light (scene, new Content::SpotLight() )
, m_InnerCone (NULL)
, m_OuterCone (NULL)
, m_FirstEntry( true )
{
...
SpotLight::SpotLight(Core::Scene* scene, Content::SpotLight* light)
...
void SpotLight::Initialize()
{
    ...
    if (!m_InnerCone)
    {
        m_InnerCone = new Core::PrimitiveCone ( m_Owner->GetViewport()->GetResources() );
    ...
    if (!m_OuterCone)
    {
        m_OuterCone = new Core::PrimitiveCone ( m_Owner->GetViewport()->GetResources() );
...
void SpotLight::SetScale( const Math::Scale& value )
{
    ...
    if( m_FirstEntry )
    {
        ...
        float maximum = MAX( MAX( abs(value.x), abs(value.y) ), abs(value.z) );
...
{
    const Core::SpotLight*    lSpotLight  = Reflect::ConstAssertCast<Core::SpotLight>( object );
...
void SpotLight::DrawInner( IDirect3DDevice9* device, DrawArgs* args, const SceneNode* object )
{
    ...
    const Core::InstanceType* type = Reflect::ConstAssertCast<Core::InstanceType>( spotLight->GetNodeType() );
