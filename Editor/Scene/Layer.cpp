...
{
  Reflect::RegisterClass< Editor::Layer >( TXT( "Editor::Layer" ) );
...
{
  Reflect::UnregisterClass< Editor::Layer >();
}

///////////////////////////////////////////////////////////////////////////////
// Constructor
// 
Layer::Layer( Editor::Scene* scene, Content::Layer* layer )
: Editor::SceneNode( scene, layer )
{
...
OS_SelectableDumbPtr Layer::GetMembers()
{
  ...
  for each (Editor::SceneNode* n in m_Descendants)
  {
    members.Append(n);
  }
...
void Layer::CreatePanel( CreatePanelArgs& args )
{
  ...
  args.m_Generator->PushPanel( TXT( "Layer" ), true);
  {
    args.m_Generator->PushContainer();
    {
      ...
      args.m_Generator->AddColorPicker< Editor::Layer, Math::Color3 >( args.m_Selection, &Layer::GetColor, &Layer::SetColor );
    }
...
void Layer::BuildUnionAndIntersection( PropertiesGenerator* generator, const OS_SelectableDumbPtr& selection, tstring& unionStr, tstring& intersectionStr )
{
  ...
  if ( generator )
  {
    ...
    for ( bool isFirstLayer = true; selItr != selEnd; ++selItr, isFirstLayer = false )
    {
      ...
      // If it's a layer
      Editor::Layer* layer = Reflect::ObjectCast< Editor::Layer >( *selItr );
      if ( layer )
      {
    ...
    {
      Editor::SceneNode* node = intersectionItr->second;
