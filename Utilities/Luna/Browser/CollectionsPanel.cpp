...
BEGIN_EVENT_TABLE( CollectionsPanel, CollectionsPanelGenerated )
  ...
  EVT_MENU( BrowserMenu::ExportCollection, CollectionsPanel::OnExportCollection )
...
void CollectionsPanel::OnMyCollectionsMenu( wxTreeEvent& event )
{
  ...
  if( item != rootID )
  {
    ...
    if ( collection )
    {
      ...
      menu.Append( ID_ExportCollection, BrowserMenu::Label( ID_ExportCollection ), BrowserMenu::Label( ID_ExportCollection ), wxITEM_NORMAL );
      menu.AppendSeparator();
      ...
      menu.Enable( ID_ExportCollection, true );
...
void CollectionsPanel::OnNewCollection( wxCommandEvent& event )
{
  ...
  M_CollectionToItemID::iterator findItem = m_CollectionToItemIDs.find( collection->GetID() );
...
void CollectionsPanel::OnOpenCollection( wxCommandEvent& event )
{
  ...
  browserDlg.AddFilter( FinderSpecs::Luna::ASSET_COLLECTION_IRB_DECORATION.GetDialogFilter() );
  browserDlg.AddFilter( FinderSpecs::Luna::ASSET_COLLECTION_XML_DECORATION.GetDialogFilter() );
  browserDlg.SetFilterIndex( FinderSpecs::Luna::ASSET_COLLECTION_IRB_DECORATION.GetDialogFilter() );
...
///////////////////////////////////////////////////////////////////////////////
void CollectionsPanel::OnExportCollection( wxCommandEvent& event )
{
  UIToolKit::SortTreeCtrl* treeCtrl = m_MyCollectionsTreeCtrl;
  wxTreeItemId rootID = treeCtrl->GetRootItem();
  ...
  if ( collection )
  {
    std::string defaultDir( collection->GetFilePath() );
    FileSystem::StripLeaf( defaultDir );
    
    std::string defaultFile( FileSystem::GetLeaf( collection->GetFilePath() ) );
    FileSystem::StripExtension( defaultFile );
    
    UIToolKit::FileDialog browserDlg( this, BrowserMenu::Label( ID_ExportCollection ), defaultDir.c_str(), defaultFile.c_str(), "",
    ...
    if ( browserDlg.ShowModal() == wxID_OK )
    {
      m_CollectionManager->ExportCollection( collection, browserDlg.GetPath().c_str() );
    }
  }
}
...
void CollectionsPanel::OnAddToCollection( wxCommandEvent& event )
{
  ...
  if ( !files.empty() )
  {
    ...
    {
      tuid assetID = ( *fileItr )->GetFileID();
      collection->AddAssetID( assetID );
    }
    collection->Thaw();
  }
}
...
void CollectionsPanel::OnRemoveFromCollection( wxCommandEvent& event )
{
  ...
  if ( !files.empty() )
  {
    ...
    {
      tuid assetID = ( *fileItr )->GetFileID();
      collection->RemoveAssetID( assetID );
    }
    collection->Thaw();
  }
}
...
///////////////////////////////////////////////////////////////////////////////
bool CollectionsPanel::GetAssetIDsFromClipBoard( Inspect::ReflectClipboardData* clipboardData, S_tuid& assetIDs, std::string& lastAssetName )
{
  bool ok = true;
...
void CollectionsPanel::UpdateCollections()
{
  ...
  {   
    ...
    {    
      ...
      m_CollectionToItemIDs.insert( M_CollectionToItemID::value_type( collection->GetID(), itemID ) );
    }
