...
ObjectPtr Object::DEFAULT_BASE(new Object(LITESQL_L("litesql::Persistent"),LITESQL_L("")));

Field::counted_ptr Object::ID_FIELD(new Field(LITESQL_L("id"), A_field_type_integer, LITESQL_L(""), A_field_indexed_false, A_field_unique_false));
Field::counted_ptr Object::TYPE_FIELD(new Field(LITESQL_L("type"), A_field_type_string, LITESQL_L(""), A_field_indexed_false, A_field_unique_false));
...
static void initSchema(DatabasePtr& db,
    ...
    for (size_t i = 0; i < objects.size(); i++) {
        Object& o = *objects[i];
        std::map<LITESQL_String, Database::DBField::counted_ptr> fldMap;
...
void init(DatabasePtr& db,
          ObjectSequence& objects,
          Relation::sequence& relations) {
    std::map<LITESQL_String, ObjectPtr> objMap;
    ...
    // std::sort objects of relations alphabetically (ascii)

    for (Relation::sequence::const_iterator it_rel=relations.begin();it_rel!=relations.end();it_rel++)
    {
        std::sort((*it_rel)->related.begin(), (*it_rel)->related.end(),Relate::CompareByObjectName());
    }
