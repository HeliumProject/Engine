...
, m_VaultFrame( browserFrame )
{
    ...
    Helium::Path thumbnailPath( m_RootDirectory + TXT( "/.thumbnails/" ) );
...
u32 ResultsPanel::GetSelectedPaths( std::set< Helium::Path >& paths )
{
    switch ( m_CurrentMode )
    {
    case ViewModes::Thumbnail:
...
u32 ResultsPanel::GetNumFiles() const
{
    ...
    if ( results )
    {
        const std::map< u64, Helium::Path >& paths = results->GetPathsMap();
        for( std::map< u64, Helium::Path >::const_iterator itr = paths.begin(), end = paths.end(); itr != end; ++itr )
        {
