...
//
////////////////////////////////////////////////////////////////////////////////////////////////
void ProcessMip(const Texture* mip, u32 mip_level, u32 face, OutputColorFormat format, DXTOptions* dxt_options, u8* work_buffer, bool convert_to_srgb)
{
  ...
  u8* native_data = Texture::GenerateFormatData(mip, conversion_format, face, convert_to_srgb);
...
bool IG::DXTGenerateMipSet(const Texture* top_mip, DXTOptions* dxt_options)
{
  ...
  const MipGenOptions*    c_mip_gen_opts[4] = { dxt_options->m_mip_gen_options[Texture::R],
                                                dxt_options->m_mip_gen_options[Texture::G],
                                                dxt_options->m_mip_gen_options[Texture::B],
                                                dxt_options->m_mip_gen_options[Texture::A]};
  ...
  const bool              c_convert_to_srgb =  c_mip_gen_opts[Texture::R]->m_ConvertToSrgb;
  //Current face
  u32                     face              = dxt_options->m_face;
  //Size limit
  ...
  const Texture*  prev_mip  = top_mip;
  Texture*        curr_mip  = NULL;

  //Mips' separate post filters
  const PostMipImageFilter  c_mip_post_filters[]  = { c_mip_gen_opts[Texture::R]->m_PostFilter,
                                                      c_mip_gen_opts[Texture::G]->m_PostFilter,
                                                      c_mip_gen_opts[Texture::B]->m_PostFilter,
                                                      c_mip_gen_opts[Texture::A]->m_PostFilter};

  //Mips' separate generation filters
  const FilterType          c_mip_gen_filters[]   = { c_mip_gen_opts[Texture::R]->m_Filter,
                                                      c_mip_gen_opts[Texture::G]->m_Filter,
                                                      c_mip_gen_opts[Texture::B]->m_Filter,
                                                      c_mip_gen_opts[Texture::A]->m_Filter};

  //UV wrap address modes
  const UVAddressMode       c_u_wrap_mode         =  c_mip_gen_opts[Texture::R]->m_UAddressMode;
  const UVAddressMode       c_v_wrap_mode         =  c_mip_gen_opts[Texture::R]->m_VAddressMode;

  //Determine if we need to apply the mips' post filters
  u32 use_mip_post_filters  = c_mip_post_filters[Texture::R] +  c_mip_post_filters[Texture::G] +
                              c_mip_post_filters[Texture::B] +  c_mip_post_filters[Texture::A];

  //Determine how many mip levels to generate (volume texture currently don't support mip maps)
  u32 mip_count             = top_mip->IsVolumeTexture() ? 1 : c_mip_gen_opts[Texture::R]->m_Levels;
  ...
  for(u32 i = 1; i < mip_count; ++i)
  {
    ...
    u32 pass_sum  = c_mip_gen_opts[Texture::R]->m_ApplyPostFilter[i] +
                    c_mip_gen_opts[Texture::G]->m_ApplyPostFilter[i] +
                    c_mip_gen_opts[Texture::B]->m_ApplyPostFilter[i] +
                    c_mip_gen_opts[Texture::A]->m_ApplyPostFilter[i];
