...
void Tracker::SetDirectory( const Nocturnal::Directory& directory )
{
    m_Directory = directory;
}

const Nocturnal::Directory& Tracker::GetDirectory() const
{
    return m_Directory;
}
...
void Tracker::TrackEverything()
{
    ...
    while ( !m_StopTracking )
    {
        ...
        for( std::set< Nocturnal::Path >::const_iterator assetFileItr = assetFiles.begin(), assetFileItrEnd = assetFiles.end();
            !m_StopTracking && assetFileItr != assetFileItrEnd; ++assetFileItr )
        {
            ...
            try
            {
                ...
                const Nocturnal::Path& assetFilePath = (*assetFileItr);
                ...
                if ( assetFilePath.FullExtension() == TXT( "nrb" ) )
                {
                    ...
                    if ( assetClass.ReferencesObject() )
                    {
                        ...
                        Nocturnal::SearchableProperties fileProperties;
                        ...
                        std::set< Nocturnal::Path > fileReferences;
                        assetClass->GetFileReferences( fileReferences );
                        for( std::set< Nocturnal::Path >::const_iterator fileRefsItr = fileReferences.begin(),
                            fileRefsItrEnd = fileReferences.end();
                            fileRefsItr != fileRefsItrEnd; ++fileRefsItr )
                        {
                            //   see if the file has changed
                            const Nocturnal::Path& fileRefPath = (*fileRefsItr);
