...
void ThumbnailTileCreator::ThreadProc( i32 threadID )
{
    ...
    {
        for ( std::map< u64, Nocturnal::Path >::const_iterator itr = m_Results->GetPathsMap().begin(), end = m_Results->GetPathsMap().end(); itr != end; ++itr )
        {
            if ( CheckThreadLeave( threadID ) )
            {
                return;
            }
            const Nocturnal::Path& path = (*itr).second;
