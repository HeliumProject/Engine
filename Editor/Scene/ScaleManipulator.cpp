...
{
  Reflect::RegisterClass< Editor::ScaleManipulator >( TXT( "Editor::ScaleManipulator" ) );
}

void ScaleManipulator::CleanupType()
{
  Reflect::UnregisterClass< Editor::ScaleManipulator >();
}

ScaleManipulator::ScaleManipulator(const ManipulatorMode mode, Editor::Scene* scene, PropertiesGenerator* generator)
...
, m_Distance( 1.0f )
{
  ...
  m_Cube = new Editor::PrimitiveCube (m_Scene->GetViewport()->GetResources());
  m_Cube->SetSolid(true);
  m_Cube->Update();

  m_XCube = new Editor::PrimitiveCube (m_Scene->GetViewport()->GetResources());
  m_XCube->SetSolid(true);
  m_XCube->Update();

  m_YCube = new Editor::PrimitiveCube (m_Scene->GetViewport()->GetResources());
  m_YCube->SetSolid(true);
  m_YCube->Update();

  m_ZCube = new Editor::PrimitiveCube (m_Scene->GetViewport()->GetResources());
