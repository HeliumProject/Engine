///////////////////////////////////////////////////////////////////////////
// Generated by litesql-gen.exe on Fri Jan 21 20:11:22 2011 (localtime): 
// C:\WDI\Helium\Dependencies\LiteSQL\prebuilt\bin\litesql-gen.exe
...
namespace Helium
{
    ...
    case 1:
        ...
        void TrackedFileProperties::link(const litesql::Database& db, const Helium::Editor::TrackedFile& o0, const Helium::Editor::TrackedProperty& o1, tstring mValue) {
            Record values;
            Split fields;
        ...
        void TrackedFileProperties::unlink(const litesql::Database& db, const Helium::Editor::TrackedFile& o0, const Helium::Editor::TrackedProperty& o1, tstring mValue) {
        ...
        template <> litesql::DataSource<Helium::Editor::TrackedFile> TrackedFileProperties::get(const litesql::Database& db, const litesql::Expr& expr, const litesql::Expr& srcExpr) {
            SelectQuery sel;
            sel.source(table__);
            sel.result(TrackedFile.fullName());
            sel.where(srcExpr);
            return DataSource<Helium::Editor::TrackedFile>(db, Helium::Editor::TrackedFile::Id.in(sel) && expr);
        }
        template <> litesql::DataSource<Helium::Editor::TrackedProperty> TrackedFileProperties::get(const litesql::Database& db, const litesql::Expr& expr, const litesql::Expr& srcExpr) {
            SelectQuery sel;
            sel.source(table__);
            sel.result(TrackedProperty.fullName());
            sel.where(srcExpr);
            return DataSource<Helium::Editor::TrackedProperty>(db, Helium::Editor::TrackedProperty::Id.in(sel) && expr);
        }
    ...
    case 1:
        ...
        void FileReferences::link(const litesql::Database& db, const Helium::Editor::TrackedFile& o0, const Helium::Editor::TrackedFile& o1) {
            Record values;
            Split fields;
        ...
        void FileReferences::unlink(const litesql::Database& db, const Helium::Editor::TrackedFile& o0, const Helium::Editor::TrackedFile& o1) {
        ...
        litesql::DataSource<Helium::Editor::TrackedFile> FileReferences::getTrackedFile1(const litesql::Database& db, const litesql::Expr& expr, const litesql::Expr& srcExpr) {
            SelectQuery sel;
            sel.source(table__);
            sel.result(TrackedFile1.fullName());
            sel.where(srcExpr);
        ...
        litesql::DataSource<Helium::Editor::TrackedFile> FileReferences::getTrackedFile2(const litesql::Database& db, const litesql::Expr& expr, const litesql::Expr& srcExpr) {
            SelectQuery sel;
            sel.source(table__);
            sel.result(TrackedFile2.fullName());
            sel.where(srcExpr);
        ...
        const litesql::FieldType TrackedFile::MToolsVersion( TXT( "mToolsVersion_" ), TXT( "INTEGER" ),table__);
        const litesql::FieldType TrackedFile::MBroken( TXT( "mBroken_" ), TXT( "INTEGER" ),table__);
        ...
        TrackedFile::TrackedFile(const litesql::Database& db)
            : litesql::Persistent(db), id(Id), type(Type), mPath(MPath), mSize(MSize), mLastModified(MLastModified), mToolsVersion(MToolsVersion), mBroken(MBroken) {
                defaults();
        }
        TrackedFile::TrackedFile(const litesql::Database& db, const litesql::Record& rec)
            : litesql::Persistent(db, rec), id(Id), type(Type), mPath(MPath), mSize(MSize), mLastModified(MLastModified), mToolsVersion(MToolsVersion), mBroken(MBroken) {
                defaults();
                size_t size = (rec.size() > 7) ? 7 : rec.size();
                switch(size) {
    case 7: mBroken = convert<const tstring&, int>(rec[6]);
        mBroken.setModified(false);
    case 6: mToolsVersion = convert<const tstring&, int>(rec[5]);
        mToolsVersion.setModified(false);
    ...
    case 1: id = convert<const tstring&, int>(rec[0]);
        ...
        TrackedFile::TrackedFile(const TrackedFile& obj)
            : litesql::Persistent(obj), id(obj.id), type(obj.type), mPath(obj.mPath), mSize(obj.mSize), mLastModified(obj.mLastModified), mToolsVersion(obj.mToolsVersion), mBroken(obj.mBroken) {
        }
        const TrackedFile& TrackedFile::operator=(const TrackedFile& obj) {
            if (this != &obj) {
                ...
                mToolsVersion = obj.mToolsVersion;
                mBroken = obj.mBroken;
            }
        ...
        tstring TrackedFile::insert(litesql::Record& tables, litesql::Records& fieldRecs, litesql::Records& valueRecs) {
            ...
            mLastModified.setModified(false);
            fields.push_back(mToolsVersion.name());
            values.push_back(mToolsVersion);
            mToolsVersion.setModified(false);
            fields.push_back(mBroken.name());
            values.push_back(mBroken);
            mBroken.setModified(false);
        ...
        void TrackedFile::addUpdates(Updates& updates) {
            ...
            updateField(updates, table__, mToolsVersion);
            updateField(updates, table__, mBroken);
        }
        ...
        void TrackedFile::getFieldTypes(std::vector<litesql::FieldType>& ftypes) {
            ...
            ftypes.push_back(MToolsVersion);
            ftypes.push_back(MBroken);
        }
        ...
        std::auto_ptr<TrackedFile> TrackedFile::upcastCopy() {
            ...
            np->mToolsVersion = mToolsVersion;
            np->mBroken = mBroken;
        ...
        tostream & operator<<(tostream& os, TrackedFile o) {
            ...
            os << o.mToolsVersion.name() <<  TXT( " = " ) << o.mToolsVersion << std::endl;
            os << o.mBroken.name() <<  TXT( " = " ) << o.mBroken << std::endl;
            os <<  TXT( "-------------------------------------" ) << std::endl;
            return os;
        }
    ...
    case 1: id = convert<const tstring&, int>(rec[0]);
        ...
        std::vector<litesql::Database::SchemaItem> TrackerDBGenerated::getSchema() const {
            ...
            res.push_back(Database::SchemaItem( TXT( "TrackedFile_" ), TXT( "table" ), TXT( "CREATE TABLE TrackedFile_ (id_ " ) + backend->getRowIDType() +  TXT( ",type_ TEXT,mPath_ TEXT,mSize_ INTEGER,mLastModified_ INTEGER,mToolsVersion_ INTEGER,mBroken_ INTEGER)" )));
