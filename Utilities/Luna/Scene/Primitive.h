...
namespace Luna
{
  ...
  public:
    LUNA_DECLARE_TYPE(Luna::Primitive, Object);
  ...
  public:
    ...
    bool IsSolid(Luna::Camera* camera) const;
    bool IsSolid() const
    {
      return m_IsSolid;
    }
  ...
  template <class T>
  class PrimitiveTemplate : public Luna::Primitive
  {
  protected:
    //
    ...
    PrimitiveTemplate(ResourceTracker* tracker)
      : Luna::Primitive (tracker)
    {

    }

    virtual void Populate(PopulateArgs* args) NOC_OVERRIDE
    {
      switch (args->m_Type)
      {
        case ResourceTypes::Vertex:
        {
          NOC_ASSERT(m_Vertices.size() == GetElementCount());
          NOC_ASSERT(sizeof(T) == ElementSizes[ GetElementType() ]);
