...
//
Texture* Texture::LoadTGA(const void* filedata, bool convert_to_linear)
{
  ...
  Texture* surface = new Texture(tga->width,tga->height, pixelfmt);
...
bool Texture::WriteTGA(const char* fname, u32 face, bool convert_to_srgb) const
{
  ...
  u8* native_data = Texture::GenerateFormatData(this, valid_format, face, convert_to_srgb);
  if(native_data)
  {
    // write image bits
