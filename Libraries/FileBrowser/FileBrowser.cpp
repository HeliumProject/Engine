...
BEGIN_EVENT_TABLE( FileBrowser, wxWindow )
    ...
    EVT_BUTTON( ID_SearchButton, OnSearch )
    EVT_BUTTON( ID_ClearButton, OnClear )
    EVT_BEGIN_SEARCH( wxID_ANY, OnBeginSearch )
    EVT_END_SEARCH( wxID_ANY, OnEndSearch )

    EVT_TEXT_ENTER( ID_SearchQuery, OnSearch ) 
    EVT_TEXT_ENTER( ID_LookInPath, OnSearch )
    EVT_TEXT_ENTER( ID_AssetTUID, OnSearch ) 
    EVT_KEY_UP( OnChar )

    EVT_CHOICE( ID_FileType, OnFileType )

    EVT_BUTTON( ID_BrowseButton, OnBrowseButton )

    EVT_SIZE( OnSize )

    EVT_LIST_END_LABEL_EDIT( ID_ListCtrlResults, OnItemEndEdit ) 	    // Finish editing a label. This can be prevented by calling Veto().

    EVT_LIST_ITEM_ACTIVATED( ID_ListCtrlResults, OnItemActivated )     // The item has been double-clicked.
    EVT_LIST_ITEM_SELECTED( ID_ListCtrlResults, OnItemSelection ) 	    // The item has been selected.
    EVT_LIST_ITEM_DESELECTED( ID_ListCtrlResults, OnItemDeselection )  // The item has been deselected.
    EVT_LIST_ITEM_FOCUSED( ID_ListCtrlResults, OnItemSelection ) 	    // The currently focused item has changed.
    EVT_LIST_COL_CLICK( ID_ListCtrlResults, OnColClick ) 	            // A column (m_col) has been left-clicked.
...
//
class File::UpdateTimer : public wxTimer
{
public:
...
// Used to update the UI at a regular interval
//
class File::ThrobberTimer : public wxTimer
{
public:
    FileBrowser*    m_FileBrowser;
...
// Used to update the UI at a regular interval
//
class File::ShowDetailsTimer : public wxTimer
{
public:
    FileBrowser*    m_FileBrowser;
...
void FileBrowser::Init()
{
    ...
    if ( m_DisplayColumns.empty() )
    {
        ...
        m_DisplayColumns[DisplayColumnTypes::Folder]     = DisplayColumn( DisplayColumnTypes::Folder,     "In Folder",      250, FileInfo::GetDirectory );
...
void FileBrowser::OnItemSelection( wxListEvent& evt )
{
    ...
    {
        if ( m_SelectedFileInfo->m_File.Exists() )
        {
            AddPath( m_SelectedFileInfo->m_File.GetPath().Get() );
        ...
        else
        {
            ...
            error += m_SelectedFileInfo->m_File.GetPath().Get();
            wxMessageBox( error.c_str(), "Error", wxCENTER | wxOK | wxICON_ERROR, GetParent() );
...
void FileBrowser::OnItemDeselection( wxListEvent& evt )
{
    ...
    if ( m_SelectedFileInfo )
    {
        RemovePath( m_SelectedFileInfo->m_File.GetPath().Get() );
    }
...
void FileBrowser::AddResult( FileInfo* fileInfo )
{
    ...
    if ( foundFilter != NULL )
    {
        ...
        if ( ( foundExtensions.find( ".*" ) == foundExtensions.end() )
            && ( foundExtensions.find( fileInfo->m_File.GetPath().Extension() ) == foundExtensions.end() ) )
        {
            return;
        }
    }
    ...
    std::string fileExtension = fileInfo->m_File.GetPath().Extension();
    ...
    // File name
    wxString buf;
    buf.Printf( wxT( "%s" ), fileInfo->m_File.GetPath().Filename().c_str() );
...
void FileBrowser::FindManagedFiles( const std::string& searchQuery )
{
    ...
    try
    {
        File::S_Reference listOfFiles;
        File::GlobalResolver().Find( searchQuery, listOfFiles );

        ::EnterCriticalSection( &m_FoundFilesCriticalSection );

        File::S_Reference::iterator it = listOfFiles.begin();
        File::S_Reference::iterator itEnd = listOfFiles.end();
        for ( ; it != itEnd; ++it )
        {
            File::ReferencePtr fileRefPtr = (*it);
            FileInfoPtr fileInfo = new FileInfo( *fileRefPtr );
