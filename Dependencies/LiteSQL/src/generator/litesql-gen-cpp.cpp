...
const LITESQL_Char* CppGenerator::NAME=LITESQL_L("c++");
    ...
    for (size_t i = 0; i < o.fields.size(); i++) {
        ...
        if (!fld.values.empty()) {
            ...
            for (size_t i2 = 0; i2 < fld.values.size(); i2++) {
                ...
                init.body(fld.name + LITESQL_L("_values.push_back(std::make_pair<LITESQL_String, LITESQL_String>(")
...
void writeObjBaseMethods(Class& cl, const xml::Object& o) {
    ...
    for (std::map<xml::Relation::counted_ptr, xml::Relate::sequence >::const_iterator i = 
...
void writeStaticRelData(Class& cl, const xml::Relation& r) {
    ...
    for (size_t i2 = 0; i2 < r.fields.size(); i2++) {
        ...
        if (!fld.values.empty()) {
            ...
            for (size_t i2 = 0; i2 < fld.values.size(); i2++) {
                const xml::Value& val = fld.values[i2];
                initValues.body(fld.name + LITESQL_L("_values.push_back(std::make_pair(")
                          + quote(val.name) + LITESQL_L(",") + quote(val.value) + LITESQL_L("));"));    
            }
