...
void Animation::EnumerateClass( Reflect::Compositor<Animation>& comp )
{
  ...
  Reflect::Field* fieldTranslate = comp.AddField( &Animation::m_Translate, "m_Translate" );
  Reflect::Field* fieldRotate = comp.AddField( &Animation::m_Rotate, "m_Rotate" );
  Reflect::Field* fieldScale = comp.AddField( &Animation::m_Scale, "m_Scale" );
}


  u32 Animation::WindowSamples() const
  {
    // support legacy data
  ...
  u32 Animation::TotalSamples() const
  {
    // support legacy data
    if ( m_TotalSamples == 0 )
    {
  ...
  bool Animation::HasMotion( const f32 translationTolerance, const f32 scaleTolerance, const f32 rotationTolerance ) const
  {
  ...
  bool Animation::HasTranslation( const f32 translationTolerance ) const
  {
  ...
  bool Animation::HasScaling( const f32 scaleTolerance ) const
  {
  ...
  bool Animation::HasRotation( const f32 rotationTolerance ) const
  {
  ...
  void Animation::SetAllTSRSamples( const Math::Vector3& translate, const Math::Vector3& scale, const Math::Quaternion& rotate )
  {
  ...
  void Animation::ApplyTranslateAdjustment( const Math::Vector3& adjustment, u32 frame )
  {
  ...
  void Animation::ApplyParentTransform( const Content::AnimationPtr& parentAnimation, bool applyScale )
  {
    u32 totalSamples = TotalSamples();
  ...
  void Animation::ApplyInverseParentTransform( const Content::AnimationPtr& parentAnimation )
  {
    u32 totalSamples = TotalSamples();
    for ( u32 frameIndex = 0; frameIndex < totalSamples; ++frameIndex )
    {
  ...
  f32 Animation::GetLinearDistancePerFrame()
  {
