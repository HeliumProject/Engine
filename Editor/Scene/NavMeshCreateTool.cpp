...
EDITOR_DEFINE_TYPE(Editor::NavMeshCreateTool);


        //case SceneEditorIDs::ID_ToolsNavMeshWorkWithLOWRes:
        //    {
        //        Editor::NavMeshCreateTool* navMeshCreate;
        //        if ( m_SceneManager.GetCurrentScene()->GetTool() &&  m_SceneManager.GetCurrentScene()->GetTool()->GetType() == Reflect::GetType<Editor::NavMeshCreateTool>() )
        //        {
        ...
        //        else
        //        {
        //            navMeshCreate = new Editor::NavMeshCreateTool (m_SceneManager.GetCurrentScene(), m_ToolEnumerator);
...
{
  Reflect::RegisterClass< Editor::NavMeshCreateTool >( TXT( "Editor::NavMeshCreateTool" ) );
}

void NavMeshCreateTool::CleanupType()
{
  Reflect::UnregisterClass< Editor::NavMeshCreateTool >();
}

NavMeshCreateTool::NavMeshCreateTool( Editor::Scene* scene, PropertiesGenerator* generator )
...
{
  m_PunchOutTranslator = new Editor::TranslateManipulator( ManipulatorModes::Translate, scene, generator );
  m_PunchOutRotator = new Editor::RotateManipulator( ManipulatorModes::Rotate, scene, generator );
  m_PunchOutScaler = new Editor::ScaleManipulator( ManipulatorModes::Scale, scene, generator );
  ...
  m_Axes = new Editor::PrimitiveAxes (m_Scene->GetViewport()->GetResources());
  m_Axes->Update();

  m_Ring = new Editor::PrimitiveCircle (m_Scene->GetViewport()->GetResources());
  m_Ring->Update();

  m_XCone = new Editor::PrimitiveCone (m_Scene->GetViewport()->GetResources());
  m_XCone->SetSolid( true );
  m_XCone->Update();

  m_YCone = new Editor::PrimitiveCone (m_Scene->GetViewport()->GetResources());
  m_YCone->SetSolid( true );
  m_YCone->Update();

  m_ZCone = new Editor::PrimitiveCone (m_Scene->GetViewport()->GetResources());
...
#pragma TODO("Move into the tool settings")
  ...
  m_SelectionFrame = new Editor::PrimitiveFrame ( m_Scene->GetViewport()->GetResources() );
  ...
  {
    m_PunchOutVolume = Reflect::ObjectCast<Editor::Volume >( *childItr );
    if (m_PunchOutVolume)
    {
      break;
    }
  ...
  if (m_PunchOutVolume == NULL)
  {
    ...
    m_PunchOutVolume = new Editor::Volume ( m_Scene, v );
  ...
  for ( ; childItr != childEnd; ++childItr )
  {
    Editor::NavMesh* mesh = Reflect::ObjectCast< Editor::NavMesh >( *childItr );
    if ( mesh )
    {
...
{
  m_Instance = new Editor::NavMesh( m_Scene, new Content::Mesh() );
  if ( m_Instance )
  {
...
bool NavMeshCreateTool::PickManipulator( PickVisitor* pick )
{
  ...
  else
  {
    ...
    if ( m_SelectedAxes != Math::MultipleAxes::None )
    {
      m_Axes->SetColor(m_SelectedAxes, Editor::Color::ColorValueToColor(Editor::Color::YELLOW));
    }
  ...
  case EDIT_MODE_MARQUEE_SELECT:
    {
      ...
      Editor::Camera* cam = m_Scene->GetViewport()->GetCamera();
...
void NavMeshCreateTool::MouseMove( wxMouseEvent& e )
{
  if ( m_Instance.ReferencesObject() )
  {
    ...
    if ( m_Instance->GetNumberVertices() > 0 )
    {
      ...
      f32 ss_len = Editor::Camera::FarClipDistance;
...
void NavMeshCreateTool::Draw( DrawArgs* args )
{
  if ( m_EditMode == EDIT_MODE_MOVE && m_Instance->m_selected_verts.size() > 0 )
  {
    ...
    if (m_SelectedAxes == Math::MultipleAxes::All)
    {
      m_AxisMaterial.Ambient = Editor::Color::YELLOW;
    }
    else
    {
      m_AxisMaterial.Ambient = Editor::Color::LIGHTGRAY;
    }
  ...
  if ( m_EditMode == EDIT_MODE_ROTATE )
  {
    ...
    {
      D3DMATERIAL9 mat = m_AxisMaterial;
      mat.Ambient = Editor::Color::GREEN;
      mat.Diffuse.a = 0.2f;
      m_View->GetDevice()->SetMaterial(&mat);
      m_Ring->DrawFill(args);
    }
    ...
    {
      D3DMATERIAL9 mat = m_AxisMaterial;
      mat.Ambient = Editor::Color::YELLOW;
      mat.Diffuse.a = 0.2f;
      m_View->GetDevice()->SetMaterial(&mat);
      m_Ring->DrawFill(args);
    }
    ...
    {
      m_AxisMaterial.Ambient = Editor::Color::SKYBLUE;
    }
...
void NavMeshCreateTool::CreateProperties()
{
  ...
  m_Generator->PushPanel( TXT( "Create NavMesh" ), true );
  {
    m_Generator->PushContainer();
    {
      ...
      m_Generator->AddCheckBox<bool>( new Helium::MemberProperty<Editor::NavMeshCreateTool, bool> (this, &NavMeshCreateTool::GetSurfaceSnap, &NavMeshCreateTool::SetSurfaceSnap ) );
    }
    m_Generator->Pop();
    ...
    m_Generator->PushContainer();
    {
      ...
      m_Generator->AddCheckBox<bool>( new Helium::MemberProperty<Editor::NavMeshCreateTool, bool> (this, &NavMeshCreateTool::GetObjectSnap, &NavMeshCreateTool::SetObjectSnap ) );
    }
    m_Generator->Pop();
    ...
    m_Generator->PushContainer();
    {
      ...
      Inspect::Choice* choice = m_Generator->AddChoice<int>( new Helium::MemberProperty<Editor::NavMeshCreateTool, int> (this, &NavMeshCreateTool::GetPlaneSnap, &NavMeshCreateTool::SetPlaneSnap) );
