  ...
  if ( !directory.empty() )
  {
    ...
    for( V_string::const_iterator itr = directories.begin() + 1, end = directories.end(); itr != end; ++itr )
    {
      if ( ( (*currentDirectory.rbegin()) != ':' ) && ( stat( currentDirectory.c_str(), &statInfo ) != 0 ) )
      {
        ...
        {
          throw Nocturnal::Exception( "Could not create directory '%s'.", currentDirectory.c_str() );
        }
      }
...
void File::Create()
{
  ...
  if ( !f )
  {
    throw Nocturnal::Exception( "Could not create file '%s'.", m_Path.c_str() );
  }
...
bool File::GetAttributes( u32& attributes )
{
...
bool File::HasChanged()
{
  struct _stati64 stats;
  GetStats64( stats );
  return ( stats.st_mtime > m_ModTime );
}

bool File::GetStats( struct _stat& stats )
{
  return ( _stat( m_Path.Native().c_str(), &stats ) == 0 );
}

bool File::GetStats64( struct _stati64& stats )
{
