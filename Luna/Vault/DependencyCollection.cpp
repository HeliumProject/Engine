...
void DependencyCollection::InitializeCollection() 
{
    ...
    wxGetApp().GetVault().GetVaultPreferences()->AddChangedListener( Reflect::ElementChangeSignature::Delegate( this, &DependencyCollection::OnPreferencesChanged ) );
}

///////////////////////////////////////////////////////////////////////////////
void DependencyCollection::CleanupCollection() 
{
    wxGetApp().GetVault().GetVaultPreferences()->RemoveChangedListener( Reflect::ElementChangeSignature::Delegate( this, &DependencyCollection::OnPreferencesChanged ) );
...
u32 DependencyCollection::GetRecursionDepthForLoad() const
{
    ...
    if ( IsReverse() )
    {
        maxRecursionDepth = wxGetApp().GetVault().GetVaultPreferences()->GetUsageCollectionRecursionDepth();
    }
    else
    {
        maxRecursionDepth = wxGetApp().GetVault().GetVaultPreferences()->GetDependencyCollectionRecursionDepth();
    }
...
#pragma TODO( "Reimplmenet without the CacheDB" )
            //wxGetApp().GetVault().GetCacheDB()->GetAssetDependencies( m_spFileReference, assets, m_IsReverse, GetRecursionDepthForLoad() );
