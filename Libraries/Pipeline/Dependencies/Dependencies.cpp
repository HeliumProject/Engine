...
bool DependencyGraph::RegisterType( const std::string& typeName, const GetFormatVersionSignature::Delegate& callback )
{
    M_TypeNameToFormatVersionCallback::const_iterator itr = m_FormatVersionCallbacks.find( typeName );
    if ( itr != m_FormatVersionCallbacks.end() )
    {
        return false;
    }

    m_FormatVersionCallbacks[ typeName ] = callback;
}

bool DependencyGraph::UnregisterType( const std::string& typeName )
{
    unsigned int numErased = m_FormatVersionCallbacks.erase( typeName );
    return numErased > 0;
}

std::string DependencyGraph::GetFormatVersion( const std::string& typeName ) const
{
    M_TypeNameToFormatVersionCallback::const_iterator itr = m_FormatVersionCallbacks.find( typeName );
    if ( itr != m_FormatVersionCallbacks.end() )
    {
        return (*itr).second.Invoke( Nocturnal::Void() );
    }
...
void DependencyGraph::CreateSignature( const DependencyInfoPtr& file )
{
    ...
    std::string formatVersion = GetFormatVersion( file->m_TypeName );
    fileSig << formatVersion;
    outFile->m_SignatureTrace.push_back( formatVersion );
...
void DependencyGraph::CacheCommitDependency( DependencyInfoPtr file, bool fileExists )
{
    ...
    file->SetInfo();
    file->m_VersionRowID = m_GraphDB->InsertVersion( file->m_TypeName, file->m_FormatVersion );      
...
//  - the most recently cached built file and FormatVersion info for a given file
...
void DependencyGraph::CacheCommitGraph( const DependencyInfoPtr file, bool useTransaction )
{
    ...
    try
    {
        ...
        outFile->m_TypeName = file->m_TypeName;
...
DependencyInfoPtr DependencyGraph::CreateCacheCopy( const DependencyInfoPtr& dependency )
{
    ...
    if( fileInfo )
    {
        return new FileInfo( this, fileInfo->m_Path, fileInfo->m_TypeName );
    }
    ...
    if ( dataInfo )
    {
        return new DataInfo( this, dataInfo->m_Path, dataInfo->m_TypeName, dataInfo->m_Data, dataInfo->m_DataSize );
    }
