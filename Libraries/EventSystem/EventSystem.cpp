...
: m_WriteBinaryFormat( writeBinaryFormat )
{
  ...
  // m_RootDirPath, ensure directory exists
...
void EventSystem::GetUnhandledEvents( V_EventPtr &listOfEvents )
{
  ...
  if ( FileSystem::Exists( m_HandledEventsFile ) )
  {
...
void EventSystem::GetEvents( V_EventPtr& listOfEvents, bool sorted )
{
  ...
  for( ; itr != end; ++itr )
  {
    const std::string& filePath = (*itr);
    ReadBinaryEventsFile( filePath, listOfEvents, false );
  }
  ...
  {
    const std::string& filePath = (*itr);
    ReadTextEventsFile( filePath, listOfEvents, false );
  }
...
void EventSystem::ReadEventsFile( const std::string& eventsFile, V_EventPtr& listOfEvents, bool sorted )
{
  ...
  if ( FileSystem::HasExtension( eventsFile, FinderSpecs::Extension::DATA.GetExtension() ) )
  {
    ReadBinaryEventsFile( eventsFile, listOfEvents, sorted );
  }
  else if ( FileSystem::HasExtension( eventsFile, FinderSpecs::Extension::TXT.GetExtension() ) )
  {
...
void EventSystem::WriteBinaryEventsFile( const std::string& eventsFile, const V_EventPtr& listOfEvents )
{
  ...
  if ( FileSystem::Exists( eventsFile )
    && FileSystem::HasAttribute( eventsFile, FILE_ATTRIBUTE_READONLY ) )
  {
...
void EventSystem::WriteTextEventsFile( const std::string& eventsFile, const V_EventPtr& listOfEvents )
{
  ...
  if ( FileSystem::GetSize( eventsFile ) == 0 )
  {
...
void EventSystem::WriteHandledEvents( const V_EventPtr& listOfEvents )
{
  ...
  // m_HandledEventsFile, make sure it's not read-only
  if ( FileSystem::Exists( m_HandledEventsFile ) 
    && FileSystem::HasAttribute( m_HandledEventsFile, FILE_ATTRIBUTE_READONLY ) )
  {
...
void EventSystem::FlushHandledEvents()
{
  ...
  if ( FileSystem::Exists( m_HandledEventsFile ) )
  {
    std::string handledEventsWin32Name = m_HandledEventsFile;
    FileSystem::Win32Name( handledEventsWin32Name );
...
bool EventSystem::HandleEventsFileExists()
{
  ...
  return FileSystem::Exists( m_HandledEventsFile ) ? true : false ;
}
...
void EventSystem::DumpEventsToTextFile( const std::string& datFile, const std::string& txtFile )
{
  ...
  if ( datFile.empty() || !FileSystem::Exists( datFile ) )
  {
  ...
  FileSystem::MakePath( cleanTextFile, true );
...
void EventSystem::LoadEventsFromTextFile( const std::string& txtFile, const std::string& datFile )
{
  ...
  // setup the input file
  if ( txtFile.empty() || !FileSystem::Exists( txtFile ) )
  {
    throw Exception( "Input text events file [%s] doesn't exist!", txtFile.c_str() );
  }

  // setup the output file
  std::string cleanEventsFile = datFile;
  ...
  if ( FileSystem::Exists( cleanEventsFile )
    && FileSystem::HasAttribute( cleanEventsFile, FILE_ATTRIBUTE_READONLY ) )
  {
  ...
  StompEventsFile( cleanEventsFile, listOfEvents );
}
