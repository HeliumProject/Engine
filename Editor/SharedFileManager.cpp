...
// 
void SharedFileManager::AddFileListener( const Helium::Path& path, const SharedFileChangedSignature::Delegate& listener )
{
...
// 
void SharedFileManager::RemoveFileListener( const Helium::Path& path, const SharedFileChangedSignature::Delegate& listener )
{
...
// 
void SharedFileManager::FireEvent( const Helium::Path& path, FileActions::FileAction action, const SharedFileChangedSignature::Delegate& emitter )
{
    M_SharedFileChangedEvent::iterator found = m_Events.find( path.Hash() );
    if ( found != m_Events.end() )
    {
