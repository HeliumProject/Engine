...
void PointerFixup::ChangeDestination( const StrongBufferLocation& new_destination )
{
...
void PointerFixup::EraseIncoming( const WeakBufferLocation& source_location ) const
{
	if ( m_Destination.second != NULL )
	{
		std::vector< WeakBufferLocation >& fixups = m_Destination.second->GetIncomingFixups();
		std::vector< WeakBufferLocation >::const_iterator newEnd = std::remove( fixups.begin(), fixups.end(), source_location );
		fixups.erase( newEnd, fixups.end() );
	}
}

bool PointerFixup::DoFixup( const WeakBufferLocation& source_location )
{
	...
	if ( m_Destination.second != NULL )
	{
		...
		*source_addr = (void*)(destination_buffer->GetData() + destination_offset);
		...
		std::vector< WeakBufferLocation >& fixups = destination_buffer->GetIncomingFixups();
		if ( std::find( fixups.begin(), fixups.end(), source_location ) == fixups.end() )
		{
			fixups.push_back( source_location );
		}
...
void OffsetFixup::ChangeDestination( const StrongBufferLocation& new_destination )
{
	bool had_ref = m_HasReference;
	SmartBuffer* old_buffer = m_Destination.second;
...
void OffsetFixup::EraseIncoming( const WeakBufferLocation& source_location ) const
{
	if ( m_Destination.second != NULL )
	{
		std::vector< WeakBufferLocation >& fixups = m_Destination.second->GetIncomingFixups();
...
bool OffsetFixup::DoFixup( const WeakBufferLocation& source_location )
{
	HELIUM_ASSERT( source_location.second != NULL );
	HELIUM_ASSERT( source_location.first <= source_location.second->GetSize() );
...
bool VTableFixup::DoFixup( const WeakBufferLocation& source_location )
{
	HELIUM_ASSERT( source_location.second != NULL );
	HELIUM_ASSERT( source_location.first <= source_location.second->GetSize() );
	...
	if ( m_Size + size > m_Capacity )
	{
		...
		else
		{
			...
			if ( ptr != m_Data )
			{
				...
				{
					std::vector< WeakBufferLocation >::iterator itr = m_IncomingFixups.begin();
					std::vector< WeakBufferLocation >::iterator end = m_IncomingFixups.end();
					for ( ; itr != end; ++itr )
					{
						WeakBufferLocation& source = (*itr);

						// get the target location from the source buffer
						std::map< uint32_t, FixupPtr >::iterator found = source.second->m_OutgoingFixups.find( source.first );
...
void SmartBuffer::CollectChildren( std::vector< SmartBufferPtr >& buffers )
{
	// we don't bother collecting empty buffers
	if ( GetSize() > 0 )
	{
		SmartBufferPtr thisBuffer ( this );
		if ( std::find( buffers.begin(), buffers.end(), thisBuffer ) == buffers.end() )
		{
			buffers.push_back( thisBuffer );

			StrongBufferLocation destination;

			std::map< uint32_t, FixupPtr >::const_iterator itr = m_OutgoingFixups.begin();
			std::map< uint32_t, FixupPtr >::const_iterator end = m_OutgoingFixups.end();
			for( ; itr != end; ++itr )
			{
...
void SmartBuffer::InheritFixups( const SmartBufferPtr& buffer, uint32_t offset )
{
	...
	if ( !buffer->GetIncomingFixups().empty() )
	{
		...
		std::vector< WeakBufferLocation > incoming_fixup_copy = buffer->GetIncomingFixups();

		std::vector< WeakBufferLocation >::const_iterator itr = incoming_fixup_copy.begin();
		std::vector< WeakBufferLocation >::const_iterator end = incoming_fixup_copy.end();
		for( ; itr != end; ++itr )
		{
			// this is the old incoming fixup data
			WeakBufferLocation source_location = (*itr);

			// get the target location from the source buffer
			std::map< uint32_t, FixupPtr >::iterator i = source_location.second->m_OutgoingFixups.find( source_location.first );
			...
			StrongBufferLocation old_destination;
			...
			StrongBufferLocation new_destination (old_destination.first + offset, this);
	...
	{
		std::map< uint32_t, FixupPtr > outgoing_fixups_copy = buffer->m_OutgoingFixups;

		std::map< uint32_t, FixupPtr >::const_iterator itr = outgoing_fixups_copy.begin();
		std::map< uint32_t, FixupPtr >::const_iterator end = outgoing_fixups_copy.end();
		for( ; itr != end; ++itr )
		{
			// get a pointer to the fixup
			...
			WeakBufferLocation old_source_location ( (*itr).first, buffer );
			AddFixup( old_source_location, NULL );

			// we need to offset the previous fixup by the correct offset in the current buffer
			WeakBufferLocation new_source_location ( offset + (*itr).first, this );
...
void SmartBuffer::Dump()
{
	...
	{
		// first we copy the incoming fixups, since we are messing with that map 
		std::vector< WeakBufferLocation >::const_iterator itr = m_IncomingFixups.begin();
		std::vector< WeakBufferLocation >::const_iterator end = m_IncomingFixups.end();
		for( ; itr != end; ++itr )
		{
			const WeakBufferLocation& source_location = (*itr);
...
bool SmartBuffer::AddFixup( const WeakBufferLocation& source, const FixupPtr& fixup )
{
...
bool SmartBuffer::AddOffsetFixup( const StrongBufferLocation& source, const StrongBufferLocation& destination,bool absolute)
{
	...
	WeakBufferLocation source_location( source_offset, source_buffer );
...
bool SmartBuffer::AddPointerFixup( const StrongBufferLocation& source, const StrongBufferLocation& destination, uint32_t size )
{
	// double check the source data
	HELIUM_ASSERT( source.second != NULL );
	HELIUM_ASSERT( source.first <= source.second->m_Size );
...
bool SmartBuffer::AddVTableFixup( const StrongBufferLocation& source, uint32_t class_index, uint32_t size )
{  
	// double check the source data
	HELIUM_ASSERT( source.second != NULL );
	HELIUM_ASSERT( source.first <= source.second->m_Size );
...
void SmartBuffer::Write(const StrongBufferLocation& pointer,const void* src,uint32_t size)
{
...
void SmartBuffer::WriteI8(const StrongBufferLocation& pointer,int8_t val)
{
	Write(pointer,&val,sizeof(int8_t));
}

void SmartBuffer::WriteU8(const StrongBufferLocation& pointer,uint8_t val)
{
	Write(pointer,&val,sizeof(uint8_t));
}

void SmartBuffer::WriteI16(const StrongBufferLocation& pointer,int16_t val)
{
	val = ConvertEndian(val,pointer.second->IsPlatformBigEndian());
	Write(pointer,&val,sizeof(int16_t));
}

void SmartBuffer::WriteU16(const StrongBufferLocation& pointer,uint16_t val)
{
	val = ConvertEndian(val,pointer.second->IsPlatformBigEndian());
	Write(pointer,&val,sizeof(uint16_t));
}

void SmartBuffer::WriteI32(const StrongBufferLocation& pointer,int32_t val)
{
	val = ConvertEndian(val,pointer.second->IsPlatformBigEndian());
	Write(pointer,&val,sizeof(int32_t));
}

void SmartBuffer::WriteU32(const StrongBufferLocation& pointer,uint32_t val)
{
	val = ConvertEndian(val,pointer.second->IsPlatformBigEndian());
	Write(pointer,&val,sizeof(uint32_t));
}

void SmartBuffer::WriteI64(const StrongBufferLocation& pointer,int64_t val)
{
	val = ConvertEndian(val,pointer.second->IsPlatformBigEndian());
	Write(pointer,&val,sizeof(int64_t));
}

void SmartBuffer::WriteU64(const StrongBufferLocation& pointer,uint64_t val)
{
	val = ConvertEndian(val,pointer.second->IsPlatformBigEndian());
	Write(pointer,&val,sizeof(uint64_t));
}

void SmartBuffer::WriteF32(const StrongBufferLocation& pointer,float32_t val)
{
	uint32_t v = ConvertEndianFloatToU32(val,pointer.second->IsPlatformBigEndian());
	Write(pointer,&v,sizeof(v));
}

void SmartBuffer::WriteF64(const StrongBufferLocation& pointer,float64_t val)
{
