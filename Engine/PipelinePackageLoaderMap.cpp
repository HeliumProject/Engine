...
{
    /// Constructor.
    PipelinePackageLoaderMap::PipelinePackageLoaderMap()
    {
    }
    ...
    PipelinePackageLoaderMap::~PipelinePackageLoaderMap()
    {
        ConcurrentHashMap< AssetPath, PipelinePackageLoader* >::ConstAccessor loaderAccessor;
        if( m_packageLoaderMap.First( loaderAccessor ) )
        {
            do
            {
                PipelinePackageLoader* pLoader = loaderAccessor->Second();
    ...
    PipelinePackageLoader* PipelinePackageLoaderMap::GetPackageLoader( AssetPath path )
    {
        ...
        while( !packagePath.IsPackage() )
        {
            ...
            if( packagePath.IsEmpty() )
            {
                HELIUM_TRACE(
                    TraceLevels::Error,
                    ( TXT( "PipelinePackageLoaderMap::GetPackageLoader(): Cannot resolve package loader for \"%s\", as it " )
        ...
        ConcurrentHashMap< AssetPath, PipelinePackageLoader* >::ConstAccessor constMapAccessor;
        if( m_packageLoaderMap.Find( constMapAccessor, packagePath ) )
        {
            PipelinePackageLoader* pLoader = constMapAccessor->Second();
            HELIUM_ASSERT( pLoader );
        ...
        ConcurrentHashMap< AssetPath, PipelinePackageLoader* >::Accessor mapAccessor;
        bool bInserted = m_packageLoaderMap.Insert(
            mapAccessor,
            KeyValue< AssetPath, PipelinePackageLoader* >( packagePath, NULL ) );
        if( bInserted )
        {
            // Entry added, so create and initialize the package loader.
            PipelinePackageLoader* pLoader = new PipelinePackageLoader;
            HELIUM_ASSERT( pLoader );
            ...
            if( !bInitResult )
            {
                HELIUM_TRACE(
                    TraceLevels::Error,
                    TXT( "PipelinePackageLoaderMap::GetPackageLoader(): Failed to initialize package loader for \"%s\".\n" ),
        ...
        // Easy fix may be to allocate and construct (but don't completely init) an PipelinePackageLoader, and try
        ...
        PipelinePackageLoader* pLoader = mapAccessor->Second();
        HELIUM_ASSERT( pLoader );
    ...
    void PipelinePackageLoaderMap::TickPackageLoaders()
    {        
        /// Cached list of package loaders iterated over in Tick() (separated to avoid deadlocks with concurrent hash
        /// map access).
        DynamicArray< PipelinePackageLoader* > m_packageLoaderTickArray;
        ...
        {
            PipelinePackageLoader* pLoader = m_packageLoaderTickArray[ loaderIndex ];
            HELIUM_ASSERT( pLoader );
            pLoader->Tick();
        }
