...
const float Luna::Camera::NearClipDistance = 0.05f;
const float Luna::Camera::FarClipDistance = 10000.0f;
const float Luna::Camera::FieldOfView = 72.0f * Math::DegToRad;

LUNA_DEFINE_TYPE(Luna::Camera);
...
void Camera::Setup(ProjectionMode mode, const Vector3& dir, const Vector3& up)
{
  ...
  {
    m_Orientation.SetBasis(Luna::UpAxis, Vector4(up));
    m_Orientation.SetBasis(Luna::SideAxis, Vector4(dir.Cross(up)));
    m_Orientation.SetBasis(Luna::OutAxis, Vector4(Vector3::Zero - dir));
