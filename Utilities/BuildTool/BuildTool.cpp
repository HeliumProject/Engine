...
void Report(Asset::AssetClass* assetClass)
{
  ...
  {
    std::string line;
    std::fstream file ( FinderSpecs::Debug::ERROR_FILE.GetFile( assetClass->GetBuiltDir() ).c_str() );
    if ( !file.fail() )
    {
...
///////////////////////////////////////////////////////////////////////////////
bool Build( const S_tuid& assetIds, const V_string& options, bool view )
{
  bool success = true;

  for ( S_tuid::const_iterator itr = assetIds.begin(), end = assetIds.end(); itr != end; ++itr )
  {
    tuid assetId = *itr;
    ...
    {
      assetClass = AssetClass::FindAssetClass( assetId );
      ...
      {
        throw Nocturnal::Exception( "Unable to locate asset '" TUID_HEX_FORMAT "'", assetId );
      }
    }
    else
    {
  ...
  if ( view )
  {
    if ( assetIds.size() > 1 )
    {
      Console::Error( "Cannot view more than one asset!\n" );
    }
    else
    {
      AssetClassPtr assetClass = AssetClass::FindAssetClass( *assetIds.begin() );

      std::string region;
      if ( !g_Regions.empty() )
      {
        region = g_Regions.front();
        if ( g_Regions.size() > 1 )
        {
          Console::Error( "More than one region specified, viewing region %s\n", region.c_str() );
        }
      } 
      else 
      {
...
///////////////////////////////////////////////////////////////////////////////
bool Build( const S_tuid& assetIds, const AssetBuilder::BuilderOptionsPtr& options, bool view )
{
  bool success = true;
  ...
  for ( V_AssetClass::const_iterator itr = assets.begin(), end = assets.end(); itr != end; ++itr )
  {
    Report( *itr );
  }
  ...
  {
      AssetClassPtr assetClass = assets.front();
    ...
    std::string region;
      ...
      AssetBuilder::LevelBuilderOptions* levelOptions = Reflect::ObjectCast<AssetBuilder::LevelBuilderOptions>( options );
      if ( region.empty() && levelOptions )
      {
        if ( !levelOptions->m_Regions.empty() )
        {
          region = levelOptions->m_Regions.front();
...
{ 
  ...
  File::FindManagedFiles( g_SearchQuery, possibleMatches, maxMatches, FinderSpecs::Extension::REFLECT_BINARY.GetExtension() );
  ...
  if ( g_All )
  {
    for each ( const File::ManagedFilePtr& file in possibleMatches )
    {
      assetIds.insert( file->m_Id );
    }
  }
  else
  {
    ...
    // make it be the one to build
    assetIds.insert( possibleMatches.at( assetIndex )->m_Id );
  }

  return Build( assetIds, options, g_View );
}
...
bool RunAsBuildWorker()
{
  ...
  do
  {
    ...
    if (msg)
    {
      ...
      success &= Build( job->m_Assets, job->m_Options, job->m_View );
