...
namespace Nocturnal
{
  ...
  public:
    ...
    const wxBitmap& GetBitmap( const tstring& fileName, long type = wxBITMAP_TYPE_ANY );
    ...
    const wxAnimation& GetAnimation( const tstring& fileName, wxAnimationType type = wxANIMATION_TYPE_GIF );
    ...
    int GetImageIndex( const tstring& fileName );

    const tstring& GetNameFromImageIndex( i32 index ) const;

    long GetBitmapLoadType( const tstring& fileName );
    void GetFullImagePath( const tstring& partialPath, tstring& fullPath );
