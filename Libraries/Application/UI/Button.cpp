...
// macros
//-----------------------------------------------------------
IMPLEMENT_DYNAMIC_CLASS( Nocturnal::Button, wxBitmapButton );

// ============================================================================
// implementation
// ============================================================================
//BEGIN_EVENT_TABLE( Nocturnal::Button, wxBitmapButton );
//END_EVENT_TABLE();
...
bool Button::MSWOnDraw(WXDRAWITEMSTRUCT *item)
{
    ...
    bool noMargins         = !( m_Margins.GetLeft() || m_Margins.GetRight() || m_Margins.GetTop() || m_Margins.GetBottom() );
    ...
    {
        wxRect rect( x + m_Margins.GetLeft() ,
            y + m_Margins.GetTop() ,
            width - m_Margins.GetRight() ,
            height - m_Margins.GetLeft() );
    ...
    if( !noMargins && CheckFlagValues( m_ButtonStyle, ButtonStyles::BU_BMP_BACKGROUND ) )
    {
        wxRect rect( x + m_Margins.GetLeft(),
            y + m_Margins.GetTop(),
            width - (m_Margins.GetRight() + m_Margins.GetLeft()),
            height - (m_Margins.GetTop() + m_Margins.GetBottom()) );
    ...
    if( CheckFlagValues(m_ButtonStyle, ButtonStyles::BU_BITMAP|ButtonStyles::BU_LABEL) && !(m_ButtonStyle & ButtonStyles::BU_BMP_BACKGROUND) )    // has both
    {
            ...
            || ( m_BitmapAlignment == BitmapAlignments::BU_BMP_RIGHT ) )
        {
            ...
            {
                yBmp = y + m_Margins.GetTop(); 
                yText = y + m_Margins.GetTop();
            }
    ...
    {
        xText = CheckFlagValues(m_ButtonStyle, ButtonStyles::BU_CENTER_HORIZONTAL) ? (x + ( width - wText ) * 0.5f ) : (x + m_Margins.GetLeft());
        yText = CheckFlagValues(m_ButtonStyle, ButtonStyles::BU_CENTER_VERTICAL) ?  (y + (height - hText) * 0.5f ) : (y + m_Margins.GetTop());
    }
    else if ( CheckFlagValues(m_ButtonStyle, ButtonStyles::BU_BITMAP) && !(m_ButtonStyle & ButtonStyles::BU_BMP_BACKGROUND) )
    {
        xBmp = CheckFlagValues(m_ButtonStyle, ButtonStyles::BU_CENTER_HORIZONTAL) ? (x + ( width - wBmp ) * 0.5f ) : (x + m_Margins.GetLeft());
        yBmp = CheckFlagValues( m_ButtonStyle, ButtonStyles::BU_CENTER_VERTICAL ) ? (y + (height - hBmp) * 0.5f ) : (y + m_Margins.GetTop() );
    }
    else    // Default case
    {
        xBmp = x + m_Margins.GetLeft();
        yBmp = y + m_Margins.GetTop(); 
        xText = ( m_ButtonStyle & ButtonStyles::BU_BITMAP ) ?  xBmp + wBmp : x + m_Margins.GetLeft();
        yText = y + m_Margins.GetTop();
    }
...
wxSize Button::DoGetBestSize( ) const
{
    ...
    sumWidth += m_Margins.GetLeft() + m_Margins.GetRight();
    sumHeight += m_Margins.GetTop() + m_Margins.GetBottom();
...
void Button::DrawBitmapBackground( wxBufferedDC& bufferDC, wxBitmap* bitmap, wxRect& rect )
{
    ...
    {
        int marginsVertical = m_Margins.GetTop() + m_Margins.GetBottom();
        int marginsHorizontal = m_Margins.GetLeft() + m_Margins.GetRight();
