...
{
    Reflect::RegisterClass< Editor::Entity >( TXT( "Editor::Entity" ) );
    PropertiesGenerator::InitializePanel( TXT( "Entity" ), CreatePanelSignature::Delegate( &Entity::CreatePanel ) );
}

void Entity::CleanupType()
{
    Reflect::UnregisterClass< Editor::Entity >();
}

Entity::Entity(Editor::Scene* scene)
: Editor::Instance (scene, new Asset::EntityInstance())
{
    ConstructorInit();
}

Entity::Entity(Editor::Scene* scene, Asset::EntityInstance* entity)
: Editor::Instance ( scene, entity )
{
    ConstructorInit();
}
...
SceneNodeTypePtr Entity::CreateNodeType( Editor::Scene* scene ) const
{
    // Overridden to create an entity-specific type
    Editor::EntityType* nodeType = new Editor::EntityType( scene, GetType() );
...
Editor::EntityAssetSet* Entity::GetClassSet()
{
    return m_ClassSet;
}

const Editor::EntityAssetSet* Entity::GetClassSet() const
{
    return m_ClassSet;
}

void Entity::SetClassSet(Editor::EntityAssetSet* classSet)
{
    m_ClassSet = classSet;
}
    ...
    case GraphDirections::Downstream:
        {
            ...
            if ( IsPointerVisible() )
            {
                Editor::EntityType* type = Reflect::AssertCast<Editor::EntityType>(m_NodeType);
            ...
            if ( IsGeometryVisible() )
            {
                ...
                const Editor::Scene* nested = GetNestedScene( m_Scene->GetViewport()->GetGeometryMode() );
...
{
    const Editor::Entity* entity = Reflect::ConstAssertCast<Editor::Entity>( object );

    const Editor::EntityType* type = Reflect::ConstAssertCast<Editor::EntityType>( entity->GetNodeType() );
...
void Entity::DrawBounds( IDirect3DDevice9* device, DrawArgs* args, const SceneNode* object )
{
    ...
    const Editor::EntityAssetSet* classSet = entity->GetClassSet();
...
///////////////////////////////////////////////////////////////////////////////
// Returns true if the specified panel is supported by Editor::Entity.
//
...
///////////////////////////////////////////////////////////////////////////////
// Static function for creating the UI panel that allows users to edit Editor::Entity.
//
