...
namespace Maya
{
    ...
    void ShaderBrowser::OnOK( wxCommandEvent& evt )
    {
        ...
        if ( isDone )
        {
            for each ( const std::string& shaderFile in paths )
            {
                Maya::LoadShader( m_ShaderType, shaderFile );
            }
        }
    }
...
#pragma TODO( "Verify the shader lives within the right directory structure" )
