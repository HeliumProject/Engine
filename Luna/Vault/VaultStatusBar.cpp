...
BEGIN_EVENT_TABLE( VaultStatusBar, wxStatusBar )
EVT_SIZE(VaultStatusBar::OnSize)
END_EVENT_TABLE()
...
VaultStatusBar::VaultStatusBar( wxWindow *parent )
...
, m_Throbber( new wxAnimationCtrl( this, wxID_ANY, wxNullAnimation ) )
, m_Message( new wxStaticText( this, wxID_ANY, s_TrackingInProgress ) )
{
    ...
    Connect( m_ProgressTimer.GetId(), wxEVT_TIMER, wxTimerEventHandler( VaultStatusBar::OnTimer ), NULL, this );
}

VaultStatusBar::~VaultStatusBar()
{
    Disconnect( m_ProgressTimer.GetId(), wxEVT_TIMER, wxTimerEventHandler( VaultStatusBar::OnTimer ), NULL, this );
}
...
// 
bool VaultStatusBar::TrimString( wxString& strText, wxStaticText* textField, int maxWidth, bool prefix )
{
    int curWidth;
...
void VaultStatusBar::SetStatusText( const wxString& strText, int nField )
{
    if ( nField == FieldMain )
    {
        wxRect rect;
...
void VaultStatusBar::OnSize( wxSizeEvent& args )
{
    const u32 border = 2;
...
void VaultStatusBar::OnTimer( wxTimerEvent& args )
{
}
