...
namespace Lunar
{
    ...
    static const size_t FLOAT16_EXPONENT_MAX = ( static_cast< size_t >( 1 ) << L_FLOAT16_EXPONENT_BITS ) - 1;
    static const size_t FLOAT16_MANTISSA_MAX = ( static_cast< size_t >( 1 ) << L_FLOAT16_MANTISSA_BITS ) - 1;

    static const size_t FLOAT32_EXPONENT_BIAS = 127;
    static const size_t FLOAT32_EXPONENT_MAX = ( static_cast< size_t >( 1 ) << L_FLOAT32_EXPONENT_BITS ) - 1;
    static const size_t FLOAT32_MANTISSA_MAX = ( static_cast< size_t >( 1 ) << L_FLOAT32_MANTISSA_BITS ) - 1;
    ...
    Float16 Float32To16( Float32 value )
    {
        ...
        else if( exponent < FLOAT32_EXPONENT_BIAS - FLOAT16_EXPONENT_BIAS + 1 )
        {
            ...
            uint32_t mantissa = ( 1 << L_FLOAT32_MANTISSA_BITS ) | value.components.mantissa;
            size_t mantissaShift =
                ( L_FLOAT32_MANTISSA_BITS - L_FLOAT16_MANTISSA_BITS ) +
        ...
        else
        {
            ...
            result.components.mantissa = static_cast< uint16_t >(
                value.components.mantissa >> ( L_FLOAT32_MANTISSA_BITS - L_FLOAT16_MANTISSA_BITS ) );
        }
    ...
    Float32 Float16To32( Float16 value )
    {
        ...
        else if( exponent == 0 )
        {
            ...
            {
                uint32_t mantissaShift = L_FLOAT16_MANTISSA_BITS - static_cast< uint32_t >( Log2( mantissa ) );
                result.components.exponent = FLOAT32_EXPONENT_BIAS - ( FLOAT16_EXPONENT_BIAS - 1 ) - mantissaShift;
                result.components.mantissa =
                    mantissa << ( mantissaShift + L_FLOAT32_MANTISSA_BITS - L_FLOAT16_MANTISSA_BITS );
            }
        }
        else
        {
