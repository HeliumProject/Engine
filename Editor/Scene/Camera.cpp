...
void Camera::Setup(ProjectionMode mode, const Vector3& dir, const Vector3& up)
{
  ...
  {
    m_Orientation.SetBasis(Editor::UpAxis, Vector4(up));
    m_Orientation.SetBasis(Editor::SideAxis, Vector4(dir.Cross(up)));
    m_Orientation.SetBasis(Editor::OutAxis, Vector4(Vector3::Zero - dir));
