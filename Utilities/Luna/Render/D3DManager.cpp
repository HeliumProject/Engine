...
HRESULT igDXRender::D3DManager::InitD3D(HWND hwnd,u32 back_buffer_width, u32 back_buffer_height, u32 init_flags)
{
  ...
  if (m_master_d3d==0)
  {
    ...
    if (fmt_res==S_OK)
    {
      printf("A8R8G8B8 to display format conversion available, forcing A8R8G8B8\n");
      m_back_buffer_format = D3DFMT_A8R8G8B8;
    }
    else
    {
      printf("A8R8G8B8 not available, using current display format [hr=%x]\n",fmt_res);
    }
    ...
    {
      printf("Using ref rast forcing render buffer to A8R8G8B8\n");
...
bool igDXRender::D3DManager::SaveTGA(const char* fname)
{
  ...
  {
    printf("failed to write TGA, surface format not compatible [surface is format d3dformat %x]\n",desc.Format);
    return false;
  }
  ...
  FILE * texture_file = fopen(fname, "wb");
  if (!texture_file)
  {
    surface->UnlockRect();
    surface->Release();
