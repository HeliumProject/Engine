...
void EntityType::Create()
{
    ...
    for ( ; itr != end; ++itr )
    {
        Core::EntityAssetSet* set = Reflect::ObjectCast< Core::EntityAssetSet > (itr->second);
        if (set)
        {
            set->Create();
        }
...
void EntityType::PopulateManifest(Asset::SceneManifest* manifest) const
{
    ...
    for ( ; setItr != setEnd; ++setItr )
    {
        ...
        const Core::EntityAssetSet* entitySet = Reflect::ConstObjectCast<Core::EntityAssetSet>( set );
