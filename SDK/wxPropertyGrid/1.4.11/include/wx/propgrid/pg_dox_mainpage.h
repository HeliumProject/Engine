...
/**
    @mainpage wxPropertyGrid 1.4.11 Overview
    ...
    * operator, use GetProperty() method.
    ...
    @subsection Making property labels editable

    Normally only the second column (i.e. the one at column index 1) is editable
    by user. However, you can make any column editable by calling
    wxPropertyGrid::MakeColumnEditable(index). For instance, calling that
    function with index 0 you can make property labels editable.

    Events wxEVT_PG_LABEL_EDIT_BEGIN and wxEVT_PG_LABEL_EDIT_ENDING are generated
    as appropriate. You can veto these events to prevent label editor from
    (dis)appearing.
    ...
    * wxStringProperty has a special trait: if it has value of "<composed>",
    ...
    - Following apply if wxPG_EX_MULTIPLE_SELECTION extra window style is used:\n
      - Clicking property with Shift down will add it to list of selected properties
      - Dragging with left mouse button down will select multiple properties.
