...
bool g_RevisionControlInitialized = false;
...
void AppUtils::Startup( int argc, const char** argv, bool checkVersion )
{
  if ( ++g_InitCount == 1 )
  {
    ...
    if ( !Nocturnal::GetCmdLineFlag( AppUtils::Args::Script ) )
    {
      ...
      Console::Print( "Tools: " NOCTURNAL_PROJECT_NAME " / " NOCTURNAL_VERSION_STRING " / "NOCTURNAL_BUILD_CONFIG_STRING"\n" );
...
int AppUtils::Shutdown( int code )
{
  if ( --g_InitCount == 0 )
  {
    ...
    if ( !Nocturnal::GetCmdLineFlag( AppUtils::Args::Script ) )
    {
      ...
      char buf[80];
      sprintf(buf, "Command %s", code ? "failed" : "succeeeded");
      Console::PrintString( buf, Console::Streams::Normal, Console::Levels::Default, code ? Console::Colors::Red : Console::Colors::Green );
...
{
  if ( g_RevisionControlInitialized )
  {
    return;
  }
...
#pragma TODO("Finder -Geoff")
  ...
  g_RevisionControlInitialized = true;
}
...
void AppUtils::CleanupRevisionControl()
{
  ...
  g_RevisionControlInitialized = false;
}
...
static int StandardMainEntry( int (*main)(int argc, const char** argv), int argc, const char** argv, bool checkVersion )
{
  ...
  Console::TraceFileHandle trace ( logFolder + name + ".log", AppUtils::GetTraceStreams() );
  Console::TraceFileHandle warning ( logFolder + name + "Warnings.log", Console::Streams::Warning );
  Console::TraceFileHandle error ( logFolder + name + "Errors.log", Console::Streams::Error );
