...
#pragma TODO( "Figure out why NMTVITEMCHANGE is undeclared without this block, even though this file seems to be including commctl.h just fine" )
...
// this helper class is used to help get the bitmap for the drag image.
class TreeImageListHelper : public wxImageList
{
public:
    TreeImageListHelper( TreeCtrl *treeCtrl, const wxTreeItemId &item )
    {
        m_hImageList = (WXHIMAGELIST) TreeView_CreateDragImage(GetHwndOf(treeCtrl), (HTREEITEM) item.m_pItem);
    }
};
...
static bool TrySelectionChange( TreeCtrl* treeControl, const wxTreeItemId& item, bool select )
{
...
static bool TryUnselectAll( TreeCtrl* treeControl )
{
...
// again to find them.
static inline void UnselectAllExcept(TreeCtrl* treeControl, const wxArrayTreeItemIds& selections, HTREEITEM htItem)
{
...
struct TreeViewItem : public TV_ITEM
{
    TreeViewItem(const wxTreeItemId& item,    // the item handle
...
public:
    ...
    virtual ~TreeItemParam() { delete m_data; }
...
protected:
    ...
    DECLARE_NO_COPY_CLASS(TreeItemParam)
};

// wxVirutalNode is used in place of a single root when 'hidden' root is
// specified.
class wxVirtualNode : public TreeViewItem
{
public:
    wxVirtualNode(TreeItemParam *param)
        : TreeViewItem(TVI_ROOT, 0)
    {
        m_param = param;
    }
    ...
    TreeItemParam *GetParam() const { return m_param; }
    void SetParam(TreeItemParam *param) { delete m_param; m_param = param; }

private:
    TreeItemParam *m_param;
...
{
public:
    wxTreeTraversal(const TreeCtrl *tree)
    {
        m_tree = tree;
    }
...
protected:
    const TreeCtrl *GetTree() const { return m_tree; }

private:
    bool Traverse(const wxTreeItemId& root, bool recursively);

    const TreeCtrl *m_tree;
...
{
public:
    TraverseSelections(const TreeCtrl *tree,
                       wxArrayTreeItemIds& selections)
    ...
    {
        const TreeCtrl * const tree = GetTree();
        ...
        if ( Luna::IsItemSelected(GetHwndOf(tree), HITEM(item)) )
        {
...
{
public:
    TraverseCounter(const TreeCtrl *tree,
...
IMPLEMENT_DYNAMIC_CLASS_XTI(TreeCtrl, wxControl,"wx/treectrl.h")

wxBEGIN_PROPERTIES_TABLE(TreeCtrl)
...
wxBEGIN_HANDLERS_TABLE(TreeCtrl)
wxEND_HANDLERS_TABLE()

wxCONSTRUCTOR_5( TreeCtrl , wxWindow* , Parent , wxWindowID , Id , wxPoint , Position , wxSize , Size , long , WindowStyle )
#else
IMPLEMENT_DYNAMIC_CLASS(TreeCtrl, wxControl)
#endif
...
bool TreeCtrl::Create(wxWindow *parent,
                        wxWindowID id,
                        const wxPoint& pos,
                        const wxSize& size,
                        long style,
...
TreeCtrl::~TreeCtrl()
{
    ...
    if ( m_hasAnyAttr )
    {
        WX_CLEAR_HASH_MAP(MapTreeAttr, m_attrs);
...
TreeCtrl::GetClassDefaultAttributes(wxWindowVariant variant)
{
...
bool TreeCtrl::DoGetItem(TreeViewItem *tvItem) const
{
...
void TreeCtrl::DoSetItem(TreeViewItem *tvItem)
{
...
void TreeCtrl::SetIndent(unsigned int indent)
{
    TreeView_SetIndent(GetHwnd(), indent);
}

void TreeCtrl::SetAnyImageList(wxImageList *imageList, int which)
{
    // no error return
...
void TreeCtrl::SetImageList(wxImageList *imageList)
{
...
void TreeCtrl::SetStateImageList(wxImageList *imageList)
{
...
size_t TreeCtrl::GetChildrenCount(const wxTreeItemId& item,
...
bool TreeCtrl::SetBackgroundColour(const wxColour &colour)
{
#if !wxUSE_COMCTL32_SAFELY
...
bool TreeCtrl::SetForegroundColour(const wxColour &colour)
{
#if !wxUSE_COMCTL32_SAFELY
...
bool TreeCtrl::IsHiddenRoot(const wxTreeItemId& item) const
{
    return HITEM(item) == TVI_ROOT && HasFlag(wxTR_HIDE_ROOT);
}

wxString TreeCtrl::GetItemText(const wxTreeItemId& item) const
{
...
void TreeCtrl::SetItemText(const wxTreeItemId& item, const wxString& text)
{
...
int TreeCtrl::GetItemImage(const wxTreeItemId& item,
                             wxTreeItemIcon which) const
{
    ...
    TreeItemParam *param = GetItemParam(item);

    return param && param->HasImage(which) ? param->GetImage(which) : -1;
}

void TreeCtrl::SetItemImage(const wxTreeItemId& item, int image,
                              wxTreeItemIcon which)
{
    ...
    TreeItemParam *data = GetItemParam(item);
    if ( !data )
        return;
...
TreeItemParam *TreeCtrl::GetItemParam(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), NULL, wxT("invalid tree item") );

    TreeViewItem tvItem(item, TVIF_PARAM);
    ...
    return (TreeItemParam *)tvItem.lParam;
}
...
void TreeCtrl::SetItemHasChildren(const wxTreeItemId& item, bool has)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
    ...
    TreeViewItem tvItem(item, TVIF_CHILDREN);
    tvItem.cChildren = (int)has;
    DoSetItem(&tvItem);
}

void TreeCtrl::SetItemBold(const wxTreeItemId& item, bool bold)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
    ...
    TreeViewItem tvItem(item, TVIF_STATE, TVIS_BOLD);
    tvItem.state = bold ? TVIS_BOLD : 0;
    DoSetItem(&tvItem);
}

void TreeCtrl::SetItemDropHighlight(const wxTreeItemId& item, bool highlight)
{
    if ( IS_VIRTUAL_ROOT(item) )
        return;

    TreeViewItem tvItem(item, TVIF_STATE, TVIS_DROPHILITED);
    tvItem.state = highlight ? TVIS_DROPHILITED : 0;
    DoSetItem(&tvItem);
}

void TreeCtrl::RefreshItem(const wxTreeItemId& item)
{
    if ( IS_VIRTUAL_ROOT(item) )
        return;
...
wxColour TreeCtrl::GetItemTextColour(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxNullColour, wxT("invalid tree item") );

    MapTreeAttr::const_iterator it = m_attrs.find(item.m_pItem);
    return it == m_attrs.end() ? wxNullColour : it->second->GetTextColour();
}

wxColour TreeCtrl::GetItemBackgroundColour(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxNullColour, wxT("invalid tree item") );
...
wxFont TreeCtrl::GetItemFont(const wxTreeItemId& item) const
{
...
void TreeCtrl::SetItemTextColour(const wxTreeItemId& item,
                                   const wxColour& col)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );

    wxTreeItemAttr *attr;
    MapTreeAttr::iterator it = m_attrs.find(item.m_pItem);
    if ( it == m_attrs.end() )
    {
        m_hasAnyAttr = true;
...
void TreeCtrl::SetItemBackgroundColour(const wxTreeItemId& item,
                                         const wxColour& col)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
void TreeCtrl::SetItemFont(const wxTreeItemId& item, const wxFont& font)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
bool TreeCtrl::IsVisible(const wxTreeItemId& item) const
{
...
bool TreeCtrl::IsExpanded(const wxTreeItemId& item) const
{
    ...
    TreeViewItem tvItem(item, TVIF_STATE, TVIS_EXPANDED);
    DoGetItem(&tvItem);

    return (tvItem.state & TVIS_EXPANDED) != 0;
}

bool TreeCtrl::IsSelected(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), false, wxT("invalid tree item") );

    TreeViewItem tvItem(item, TVIF_STATE, TVIS_SELECTED);
    DoGetItem(&tvItem);

    return (tvItem.state & TVIS_SELECTED) != 0;
}

bool TreeCtrl::IsBold(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), false, wxT("invalid tree item") );
...
wxTreeItemId TreeCtrl::GetRootItem() const
{
...
wxTreeItemId TreeCtrl::GetFirstChild(const wxTreeItemId& item,
...
wxTreeItemId TreeCtrl::GetNextChild(const wxTreeItemId& WXUNUSED(item),
                                      wxTreeItemIdValue& cookie) const
{
...
wxTreeItemId TreeCtrl::GetLastChild(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId TreeCtrl::GetNextVisible(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
wxTreeItemId TreeCtrl::GetPrevVisible(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), wxTreeItemId(), wxT("invalid tree item") );
...
bool TreeCtrl::IsItemChecked(const wxTreeItemId& item) const
{
    wxCHECK_MSG( item.IsOk(), false, wxT("invalid tree item") );
...
void TreeCtrl::SetItemCheck(const wxTreeItemId& item, bool check)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
...
wxTreeItemId TreeCtrl::DoInsertAfter(const wxTreeItemId& parent,
                                       ...
                                       wxTreeItemData *data)
{
    ...
    TreeItemParam *param = new TreeItemParam;
...
void TreeCtrl::SetImageList(wxImageList *imageList, int)
{
    SetImageList(imageList);
}

int TreeCtrl::GetItemSelectedImage(const wxTreeItemId& item) const
{
    return GetItemImage(item, wxTreeItemIcon_Selected);
}

void TreeCtrl::SetItemSelectedImage(const wxTreeItemId& item, int image)
{
...
wxTreeItemId TreeCtrl::AddRoot(const wxString& text,
                                 int image, int selectedImage,
                                 wxTreeItemData *data)
{
...
wxTreeItemId TreeCtrl::DoInsertItem(const wxTreeItemId& parent,
                                      size_t index,
                                      const wxString& text,
                                      int image, int selectedImage,
                                      wxTreeItemData *data)
{
    ...
    else // find the item from index
    {
        ...
        // to the end
        wxASSERT_MSG( index == 0, _T("bad index in TreeCtrl::InsertItem") );
    }
...
void TreeCtrl::DoExpand(const wxTreeItemId& item, int flag)
{
    wxASSERT_MSG( flag == TVE_COLLAPSE ||
                  flag == (TVE_COLLAPSE | TVE_COLLAPSERESET) ||
                  flag == TVE_EXPAND   ||
                  flag == TVE_TOGGLE,
                  wxT("Unknown flag in TreeCtrl::DoExpand") );
    ...
    TreeViewItem tvItem(item, TVIF_STATE, TVIS_EXPANDEDONCE);
    tvItem.state = 0;
    DoSetItem(&tvItem);
...
void TreeCtrl::CollapseAndReset(const wxTreeItemId& item)
{
    DoExpand(item, TVE_COLLAPSE | TVE_COLLAPSERESET);
}

void TreeCtrl::Toggle(const wxTreeItemId& item)
{
...
void TreeCtrl::ExpandItem(const wxTreeItemId& item, int action)
{
...
void TreeCtrl::UnselectAll()
{
    ...
    {
        bool sendChangedEvent = Luna::TryUnselectAll( this );
...
void TreeCtrl::ScrollTo(const wxTreeItemId& item)
{
...
wxTextCtrl *TreeCtrl::GetEditControl() const
{
    return m_textCtrl;
}

void TreeCtrl::DeleteTextCtrl()
{
    if ( m_textCtrl )
    {
...
wxTextCtrl *TreeCtrl::EditLabel(const wxTreeItemId& item,
...
void TreeCtrl::DoEndEditLabel(bool discardChanges)
{
...
wxTreeItemId TreeCtrl::DoTreeHitTest(const wxPoint& point, int& flags) const
{
...
bool TreeCtrl::GetBoundingRect(const wxTreeItemId& item,
                                 wxRect& rect,
                                 bool textOnly) const
{
    RECT rc;
...
// this is just a tiny namespace which is friend to TreeCtrl and so can use
...
private:
    ...
    {
        return ((TreeItemParam*)lParam)->GetItem();
    }
};
                                       ...
                                       LPARAM htree)
{
    ...
    TreeCtrl *tree = (TreeCtrl *)htree;

    return tree->OnCompareItems(GetIdFromData(pItem1),
                                GetIdFromData(pItem2));
}

void TreeCtrl::SortChildren(const wxTreeItemId& item)
{
    wxCHECK_RET( item.IsOk(), wxT("invalid tree item") );
    ...
    //     combo for your derived TreeCtrl if will sort without
    //     OnCompareItems
    if ( GetClassInfo() == CLASSINFO(TreeCtrl) )
    {
...
bool TreeCtrl::MSWShouldPreProcessMessage(WXMSG* msg)
{
...
bool TreeCtrl::MSWCommand(WXUINT cmd, WXWORD id)
{
    if ( cmd == EN_UPDATE )
    {
...
WXLRESULT TreeCtrl::MSWWindowProc(WXUINT nMsg, WXWPARAM wParam, WXLPARAM lParam)
{
    ...
    else if ( (nMsg >= WM_MOUSEFIRST) && (nMsg <= WM_MOUSELAST) )
    {
        ...
        switch ( nMsg )
        {
            case WM_RBUTTONDOWN:
                ...
                if ( !htItem || !Luna::IsItemSelected(GetHwnd(), htItem) )
                {
                    UnselectAll();
                }
                ...
                // newly selected item
                if ( !Luna::IsItemSelected( GetHwnd(), htItem ) )
                {
                    if ( isMultiple )
                    {
                        if ( htItem )
                ...
                if ( isMultiple )
                {
                    ...
                    if ( htItem && tvht.flags & TVHT_ONITEM ) 
                    {
                        ...
                        if ( wParam & MK_SHIFT )
                        {
                            ...
                            {
                                // Select range
                                Luna::SelectRange( GetHwnd(), this, HITEM( m_htSelStart ), htItem );
                            }
                        }
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    Luna::TryUnselectAll( this ); // Does not fire the "changed" event
                ...
                case VK_RIGHT:
                    ...
                    if ( HasChildren( htSel ) )
                    {
                        if ( !IsExpanded( htSel ) )
                        {
                            ...
                            TreeViewItem tvItem(htSel, TVIF_STATE, TVIS_EXPANDEDONCE);
                            tvItem.state = 0;
                            DoSetItem(&tvItem);
...
WXLRESULT
TreeCtrl::MSWDefWindowProc(WXUINT nMsg, WXWPARAM wParam, WXLPARAM lParam)
{
    if ( nMsg == WM_CHAR )
    {
...
bool TreeCtrl::MSWOnNotify(int idCtrl, WXLPARAM lParam, WXLPARAM *result)
{
#pragma warning( push )
#pragma warning( disable: 4312 )
        ...
        case TVN_BEGINRDRAG:
            {
                ...
                event.SetItem( tv->itemNew.hItem );
                event.SetPoint( wxPoint(tv->ptDrag.x, tv->ptDrag.y) );
        ...
        case TVN_BEGINLABELEDIT:
            {
                ...
                m_idEdited = info->item.hItem;
                event.SetItem( info->item.hItem );
                event.SetLabel( info->item.pszText );
                event.SetEditCanceled( false );
            }
            break;
        ...
        case TVN_DELETEITEM:
            {
                ...
                event.SetItem( tv->itemOld.hItem );

                if ( m_hasAnyAttr )
                {
                    MapTreeAttr::iterator it = m_attrs.find(tv->itemOld.hItem);
                    if ( it != m_attrs.end() )
                    {
                        delete it->second;
                        m_attrs.erase(it);
        ...
        case TVN_ENDLABELEDIT:
            {
                ...
                event.SetItem( info->item.hItem );
                event.SetLabel( info->item.pszText );
                event.SetEditCanceled( info->item.pszText == NULL );
                break;
            }
        ...
        case TVN_KEYDOWN:
            {
                ...
                event.SetKeyEvent(CreateKeyEvent(wxEVT_KEY_DOWN,
                                                 keyCode,
                                                 lParam,
                                                 wParam));
        ...
        case TVN_SELCHANGINGW:
            if ( !HasFlag( wxTR_MULTIPLE ) )
            {
                    ...
                    hdr->code == TVN_SELCHANGEDW)
                {
                    ...
                    event.SetItem( tv->itemNew.hItem );
                    event.SetOldItem( tv->itemOld.hItem );
                }
                else
                {
                    ...
                    case CDDS_ITEMPREPAINT:
                        {
                            ...
                            TreeViewItem tvItem((void *)nmcd.dwItemSpec,
        ...
        case NM_RCLICK:
            {
                ...
                if ( TreeView_HitTest(GetHwnd(), &tvhti) )
                {
                    ...
                    {
                        event.SetItem( tvhti.hItem );
                        eventType = (int)hdr->code == NM_DBLCLK
                                    ? wxEVT_COMMAND_TREE_ITEM_ACTIVATED
                                    : wxEVT_COMMAND_TREE_ITEM_RIGHT_CLICK;

                        event.SetPoint( wxPoint( tvhti.pt.x, tvhti.pt.y ) );
                    }
    ...
    if ( event.GetItem().IsOk() )
        event.SetClientObject(GetItemData(event.GetItem()));
    ...
    switch ( hdr->code )
    {
        ...
        case TVN_BEGINRDRAG:
            ...
            if ( event.IsAllowed() )
            {
                ...
                TreeImageListHelper treeImageListHelper(this, event.GetItem());
                wxASSERT_MSG( treeImageListHelper.GetImageCount(), _T("tree image list has no images!") );

                m_dragImage = new wxDragImage(treeImageListHelper.GetBitmap(0));
        ...
        case TVN_DELETEITEM:
            {
                ...
                TreeItemParam *param =
                        (TreeItemParam *)tv->itemOld.lParam;
                delete param;
        ...
        case TVN_GETDISPINFO:
            ...
            //if ( /* !processed && */ )
            {
                ...
                const TreeItemParam * const param = GetItemParam(item);
                if ( !param )
                    break;
...
void TreeCtrl::SetState(const wxTreeItemId& node, int state)
{
    TV_ITEM tvi;
    ...
    wxCHECK_RET( state < m_imageListState->GetImageCount(),
                 _T("TreeCtrl::SetState(): item index out of bounds") );
...
int TreeCtrl::GetState(const wxTreeItemId& node)
{
    TV_ITEM tvi;
    tvi.hItem = (HTREEITEM)node.m_pItem;
    tvi.mask = TVIF_STATE;
