...
{
  Reflect::RegisterClass< Editor::Point >( TXT( "Editor::Point" ) );
  PropertiesGenerator::InitializePanel( TXT( "Point" ), CreatePanelSignature::Delegate( &Point::CreatePanel ) );
}

void Point::CleanupType()
{
  Reflect::UnregisterClass< Editor::Point >();
}

Point::Point(Editor::Scene* scene, Content::Point* data) 
: Editor::HierarchyNode(scene, data)
{
}
...
void Point::CreatePanel( CreatePanelArgs& args )
{
  args.m_Generator->PushPanel( TXT( "Point" ), true);
  {
    ...
    args.m_Generator->AddValue<Editor::Point, f32>( args.m_Selection, &Point::GetPositionX, &Point::SetPositionX );
    args.m_Generator->AddValue<Editor::Point, f32>( args.m_Selection, &Point::GetPositionY, &Point::SetPositionY );
    args.m_Generator->AddValue<Editor::Point, f32>( args.m_Selection, &Point::GetPositionZ, &Point::SetPositionZ );
