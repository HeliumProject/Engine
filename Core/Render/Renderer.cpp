...
///////////////////////////////////////////////////////////////////////////////////////////////////
void Renderer::DrawEnvironment(RenderScene* scene,RenderEnvironment* env,D3DXMATRIX& viewproj)
{
...
///////////////////////////////////////////////////////////////////////////////////////////////////
void Renderer::DrawWireframe(RenderScene* scene,RenderMesh* mesh)
{
    IDirect3DDevice9* device = GetD3DDevice();
...
///////////////////////////////////////////////////////////////////////////////////////////////////
void Renderer::DrawNormals(RenderScene* scene,RenderMesh* mesh)
{
    IDirect3DDevice9* device = GetD3DDevice();
...
///////////////////////////////////////////////////////////////////////////////////////////////////
void Renderer::DrawTangents(RenderScene* scene,RenderMesh* mesh)
{
    IDirect3DDevice9* device = GetD3DDevice();
...
///////////////////////////////////////////////////////////////////////////////////////////////////
void Renderer::DrawDebug(RenderScene* scene,RenderMesh* mesh)
{
    IDirect3DDevice9* device = GetD3DDevice();
        ...
        case DRAW_MODE_DEBUG_BASEMAP:
            {
                if (sh)
                {
                    if (sh->m_textures[Texture::SAMPLER_BASE_MAP]!=0xffffffff)
                    {
                        Texture* tex = m_shader_manager.ResolveTexture(sh->m_textures[Texture::SAMPLER_BASE_MAP]);
        ...
        case DRAW_MODE_DEBUG_NORMALMAP:
            {
                if (sh)
                {
                    if (sh->m_textures[Texture::SAMPLER_NORMAL_MAP]!=0xffffffff)
                    {
                        Texture* tex = m_shader_manager.ResolveTexture(sh->m_textures[Texture::SAMPLER_NORMAL_MAP]);
                        SetTexture(device,tex,0);
                    }
                    else
                    {
        ...
        case DRAW_MODE_DEBUG_GLOSSMAP:
            {
                if (sh)
                {
                    ...
                    if (sh->m_textures[Texture::SAMPLER_GLOSS_MAP]!=0xffffffff)
                    {
                        Texture* tex = m_shader_manager.ResolveTexture(sh->m_textures[Texture::SAMPLER_GLOSS_MAP]);
                        SetTexture(device,tex,0);
                    }
                    else
                    {
        ...
        case DRAW_MODE_DEBUG_INCANMAP:
            {
                if (sh)
                {
                    if ((sh->m_textures[Texture::SAMPLER_INCAN_MAP]!=0xffffffff) && ((sh->m_flags & SHDR_FLAG_GPI_MAP)==0))
                    {
                        Texture* tex = m_shader_manager.ResolveTexture(sh->m_textures[Texture::SAMPLER_INCAN_MAP]);
                        SetTexture(device,tex,0);
                    }
                    else
                    {
        ...
        case DRAW_MODE_DEBUG_PARALLAXMAP:
            {
                if (sh)
                {
                    if ((sh->m_textures[Texture::SAMPLER_PARALLAX_MAP]!=0xffffffff)  && ((sh->m_flags & SHDR_FLAG_GPI_MAP)==0))
                    {
                        Texture* tex = m_shader_manager.ResolveTexture(sh->m_textures[Texture::SAMPLER_PARALLAX_MAP]);
                        SetTexture(device,tex,0);
                    }
                    else
                    {
        ...
        case DRAW_MODE_DEBUG_SHADER_ALPHATYPE:
            {
                ...
                if (sh)
                {
                    if (sh->m_alpha_type==RenderShader::ALPHA_BLENDED)
                    {
                        col[0]=0.5f;
                        col[1]=0.0f;
                        col[2]=0.0f;
                    }
                    else if (sh->m_alpha_type==RenderShader::ALPHA_ADDITIVE)
                    {
                        col[0]=0.5f;
                        col[1]=0.5f;
                        col[2]=0.0f;
...
///////////////////////////////////////////////////////////////////////////////////////////////////
void Renderer::DrawMesh(RenderScene* scene,RenderMesh* mesh)
{
    IDirect3DDevice9* device = GetD3DDevice();
    ...
    {
        RenderEnvironment* env = scene->ResolveEnvironmentHandle(scene->m_environment);
    ...
    for (uint32_t i=0;i<mesh->m_fragment_count;i++)
    {
        ...
        if ((sh->m_alpha_type!=RenderShader::ALPHA_OPAQUE) && (sh->m_alpha_type !=RenderShader::ALPHA_CUTOUT))
            continue;
        ...
        if (sh->m_alpha_type ==RenderShader::ALPHA_CUTOUT)
        {
        ...
        if (sh->m_textures[Texture::SAMPLER_PARALLAX_MAP]!=0xffffffff)
        {
            Texture* tex = m_shader_manager.ResolveTexture(sh->m_textures[Texture::SAMPLER_PARALLAX_MAP]);
        ...
        if (sh->m_textures[Texture::SAMPLER_INCAN_MAP]!=0xffffffff)
        {
            Texture* tex = m_shader_manager.ResolveTexture(sh->m_textures[Texture::SAMPLER_INCAN_MAP]);
    ...
    for (uint32_t i=0;i<mesh->m_fragment_count;i++)
    {
        ...
        {
        case RenderShader::ALPHA_ADDITIVE:
            device->SetRenderState(D3DRS_ALPHABLENDENABLE,true); 
            device->SetRenderState(D3DRS_ALPHATESTENABLE,false); 
            device->SetRenderState(D3DRS_SRCBLEND,D3DBLEND_SRCALPHA);
            device->SetRenderState(D3DRS_DESTBLEND,D3DBLEND_ONE);
            break;
        case RenderShader::ALPHA_BLENDED:
            device->SetRenderState(D3DRS_ALPHABLENDENABLE,true); 
            device->SetRenderState(D3DRS_ALPHATESTENABLE,false); 
            device->SetRenderState(D3DRS_SRCBLEND,D3DBLEND_SRCALPHA);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
void Renderer::DrawReferenceGrid(RenderScene* scene)
{
    IDirect3DDevice9* device = GetD3DDevice();
...
///////////////////////////////////////////////////////////////////////////////////////////////////
void Renderer::DrawScene(RenderScene* scene)
{
    DrawScenes( 1, &scene );
}

///////////////////////////////////////////////////////////////////////////////////////////////////
void Renderer::DrawScenes(int num, RenderScene** scenes)
{
    IDirect3DDevice9* device = GetD3DDevice();
