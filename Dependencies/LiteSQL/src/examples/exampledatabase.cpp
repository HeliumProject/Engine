...
void Person::initValues() {
    sex_values.clear();
    sex_values.push_back(std::make_pair<LITESQL_String, LITESQL_String>(LITESQL_L("Male"),LITESQL_L("0")));
    sex_values.push_back(std::make_pair<LITESQL_String, LITESQL_String>(LITESQL_L("Female"),LITESQL_L("1")));
}
...
const litesql::FieldType ThingWithMethods::Own::Id(LITESQL_L("id_"),LITESQL_L("INTEGER"),LITESQL_L("ThingWithMethods_"));
const LITESQL_String ThingWithMethods::type__(LITESQL_L("ThingWithMethods"));
const LITESQL_String ThingWithMethods::table__(LITESQL_L("ThingWithMethods_"));
const LITESQL_String ThingWithMethods::sequence__(LITESQL_L("ThingWithMethods_seq"));
const litesql::FieldType ThingWithMethods::Id(LITESQL_L("id_"),LITESQL_L("INTEGER"),table__);
const litesql::FieldType ThingWithMethods::Type(LITESQL_L("type_"),LITESQL_L("TEXT"),table__);
...
LITESQL_String ThingWithMethods::insert(litesql::Record& tables, litesql::Records& fieldRecs, litesql::Records& valueRecs) {
    tables.push_back(table__);
    litesql::Record fields;
    litesql::Record values;
    fields.push_back(id.name());
...
std::auto_ptr<ThingWithMethods> ThingWithMethods::upcast() {
    return std::auto_ptr<ThingWithMethods>(new ThingWithMethods(*this));
}
std::auto_ptr<ThingWithMethods> ThingWithMethods::upcastCopy() {
    ...
    np->type = type;
    np->inDatabase = inDatabase;
    return std::auto_ptr<ThingWithMethods>(np);
}
...
std::vector<litesql::Database::SchemaItem> ExampleDatabase::getSchema() const {
    ...
    if (backend->supportsSequences()) {
        ...
        res.push_back(Database::SchemaItem(LITESQL_L("ThingWithMethods_seq"),LITESQL_L("sequence"),LITESQL_L("CREATE SEQUENCE ThingWithMethods_seq START 1 INCREMENT 1")));
    }
    ...
    res.push_back(Database::SchemaItem(LITESQL_L("ThingWithMethods_"),LITESQL_L("table"),LITESQL_L("CREATE TABLE ThingWithMethods_ (id_ ") + backend->getRowIDType() + LITESQL_L(",type_ TEXT)")));
