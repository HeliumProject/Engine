...
namespace Lunar
{
    ...
    ConstBitArrayElementProxy::ConstBitArrayElementProxy( const uint32_t& rElement, uint32_t mask )
    ...
    ConstBitArrayElementProxy::operator bool() const
    {
    ...
    bool ConstBitArrayElementProxy::operator==( bool bValue ) const
    {
    ...
    bool ConstBitArrayElementProxy::operator==( const ConstBitArrayElementProxy& rOther ) const
    {
        uint32_t bitValue = m_rElement & m_mask;
    ...
    /// @param[in] rElement  Reference to the current array element.
    /// @param[in] mask      Bit mask for the current array element.
    BitArrayElementProxy::BitArrayElementProxy( uint32_t& rElement, uint32_t mask )
    ...
    BitArrayElementProxy& BitArrayElementProxy::operator=( bool bValue )
    {
        if( bValue )
        {
            m_rElement |= m_mask;
    ...
    ///
    /// @return  Reference to this proxy object.
    BitArrayElementProxy& BitArrayElementProxy::operator=( const ConstBitArrayElementProxy& rOther )
    {
    ...
    ConstBitArrayIterator::ConstBitArrayIterator()
    {
    }
    ...
    /// @param[in] mask      Bit mask for the current array element.
    ConstBitArrayIterator::ConstBitArrayIterator( const uint32_t* pElement, uint32_t mask )
    ...
    ConstBitArrayIterator::ConstReferenceType ConstBitArrayIterator::operator*() const
    {
    ...
    ConstBitArrayIterator& ConstBitArrayIterator::operator++()
    {
        if( !( m_mask <<= 1 ) )
        {
            ++m_pElement;
    ...
    ConstBitArrayIterator ConstBitArrayIterator::operator++( int )
    {
    ...
    ///
    /// @return  Reference to this iterator.
    ConstBitArrayIterator& ConstBitArrayIterator::operator+=( ptrdiff_t offset )
    {
        if( offset < 0 )
        {
    ...
    ConstBitArrayIterator ConstBitArrayIterator::operator+( ptrdiff_t offset ) const
    {
        ConstBitArrayIterator result = *this;
        result += offset;
    ...
    ptrdiff_t ConstBitArrayIterator::operator-( const ConstBitArrayIterator& rOther ) const
    {
    ...
    bool ConstBitArrayIterator::operator==( const ConstBitArrayIterator& rOther ) const
    {
    ...
    ///
    /// @param[in] offset  Amount by which to increase this iterator's position.
    void ConstBitArrayIterator::UnsignedIncrease( size_t offset )
    {
    ...
    ///
    /// @param[in] offset  Amount by which to decrease this iterator's position.
    void ConstBitArrayIterator::UnsignedDecrease( size_t offset )
    {
        size_t elementIncrement = offset / ( sizeof( m_mask ) * 8 );
        size_t bitIncrement = offset % ( sizeof( m_mask ) * 8 );
    ...
    /// @param[in] pElement  Pointer to the current array element.
    /// @param[in] mask      Bit mask for the current array element.
    BitArrayIterator::BitArrayIterator( uint32_t* pElement, uint32_t mask )
    ...
    BitArrayIterator::ReferenceType BitArrayIterator::operator*() const
    {
    ...
    ///
    /// @return  Reference to this iterator.
    BitArrayIterator& BitArrayIterator::operator++()
    {
        if( !( m_mask <<= 1 ) )
        {
            ++m_pElement;
    ...
    ///
    /// @return  Copy of this iterator prior to incrementing.
    BitArrayIterator BitArrayIterator::operator++( int )
    {
    ...
    ///
    /// @return  Reference to this iterator.
    BitArrayIterator& BitArrayIterator::operator+=( ptrdiff_t offset )
    {
        if( offset < 0 )
        {
            UnsignedDecrease( static_cast< size_t >( -offset ) );
    ...
    ///
    /// @return  Copy of this iterator, increased by the specified amount.
    BitArrayIterator BitArrayIterator::operator+( ptrdiff_t offset ) const
    {
        BitArrayIterator result = *this;
        result += offset;
    ...
    template< typename Allocator >
    BitArray< Allocator >::BitArray( const BitArray& rSource )
        : m_pBuffer( NULL )
    ...
    template< typename Allocator >
    size_t BitArray< Allocator >::GetSize() const
    {
        return m_size;
    }
    ...
    /// @see GetSize()
    template< typename Allocator >
    bool BitArray< Allocator >::IsEmpty() const
    {
        return( m_size == 0 );
    }
    ...
    ///
    /// If the new size is smaller than the current size, allocated memory no longer in use will not be automatically
    /// freed.  If the new size is larger than the current capacity, the array memory will be reallocated according to
    /// the normal array growth rules.  This can be avoided by calling Reserve() to increase the capacity to an explicit
    ...
    /// @see GetSize()
    template< typename Allocator >
    void BitArray< Allocator >::Resize( size_t size )
    {
        if( size > m_size )
        {
            Grow( size );
    ...
    /// @see GetSize(), Reserve()
    template< typename Allocator >
    size_t BitArray< Allocator >::GetCapacity() const
    {
        return m_capacity;
    }
    ...
    /// multiple of 32 that is larger than the necessary bit capacity.  If the requested capacity is less than the
    /// current capacity, no memory will be reallocated.
    template< typename Allocator >
    void BitArray< Allocator >::Reserve( size_t capacity )
    {
    ...
    /// @see End()
    template< typename Allocator >
    typename BitArray< Allocator >::Iterator BitArray< Allocator >::Begin()
    {
    ...
    /// @see End()
    template< typename Allocator >
    typename BitArray< Allocator >::ConstIterator BitArray< Allocator >::Begin() const
    {
    ...
    /// @see Begin()
    template< typename Allocator >
    typename BitArray< Allocator >::Iterator BitArray< Allocator >::End()
    {
    ...
    /// @see Begin()
    template< typename Allocator >
    typename BitArray< Allocator >::ConstIterator BitArray< Allocator >::End() const
    {
        size_t elementIndex = m_size / ( sizeof( uint32_t ) * 8 );
        size_t bitIndex = m_size % ( sizeof( uint32_t ) * 8 );
    ...
    template< typename Allocator >
    void BitArray< Allocator >::Add( bool bValue, size_t count )
    {
    ...
    template< typename Allocator >
    void BitArray< Allocator >::SetAll( bool bValue )
    {
        size_t size = m_size;
        if( size )
        {
    ...
    template< typename Allocator >
    void BitArray< Allocator >::UnsetAll()
    {
        size_t size = m_size;
        if( size )
        {
    ...
    /// @see Pop()
    template< typename Allocator >
    size_t BitArray< Allocator >::Push( bool bValue )
    {
        size_t index = m_size;
        Add( bValue );
    ...
    template< typename Allocator >
    BitArray< Allocator >& BitArray< Allocator >::operator=( const BitArray& rSource )
    {
    ...
    BitArray< Allocator >& BitArray< Allocator >::operator=( const BitArray< OtherAllocator >& rSource )
    {
        return Assign( rSource );
    }
    ...
    /// @return  Reference to the bit at the specified index.
    template< typename Allocator >
    typename BitArray< Allocator >::ReferenceType BitArray< Allocator >::operator[]( ptrdiff_t index )
    {
    ...
    /// @return  Constant reference to the bit at the specified index.
    template< typename Allocator >
    typename BitArray< Allocator >::ConstReferenceType BitArray< Allocator >::operator[]( ptrdiff_t index ) const
    {
        return GetElement( static_cast< size_t >( index ) );
    }
    ...
    template< typename Allocator >
    void BitArray< Allocator >::Grow( size_t capacity )
    {
        if( capacity > m_capacity )
        {
    ...
    /// that only takes the same exact BitArray type (non-templated) as well.
    ///
    /// @param[in] rSource  Array from which to copy.
    ///
    /// @return  Reference to this array.
    template< typename Allocator >
    template< typename OtherAllocator >
    BitArray< Allocator >& BitArray< Allocator >::Assign( const BitArray< OtherAllocator >& rSource )
    {
        if( this != &rSource )
        {
            Clear();
