...
{
  Reflect::RegisterClass< Editor::Mesh >( TXT( "Editor::Mesh" ) );

  ZeroMemory(&s_WireMaterial, sizeof(s_WireMaterial));
  s_WireMaterial.Ambient = Editor::Color::BLACK;
  s_WireMaterial.Diffuse = Editor::Color::BLACK;
  s_WireMaterial.Specular = Editor::Color::BLACK;

  ZeroMemory(&s_FillMaterial, sizeof(s_FillMaterial));
  s_FillMaterial.Ambient = Editor::Color::DARKGRAY;
  s_FillMaterial.Diffuse = Editor::Color::DARKGRAY;
  s_FillMaterial.Specular = Editor::Color::DARKGRAY;
}

void Mesh::CleanupType()
{
  Reflect::UnregisterClass< Editor::Mesh >();
}

Mesh::Mesh( Editor::Scene* scene, Content::Mesh* mesh )
: Editor::PivotTransform ( scene, mesh )
, m_HasAlpha (false)
, m_HasColor (false)
, m_HasTexture (false)
{
...
{
  const Editor::Mesh* mesh = Reflect::ConstAssertCast< Editor::Mesh > ( object );
...
{
  device->SetMaterial( &Editor::Viewport::s_UnselectableMaterial );
}

void Mesh::SetupSelectedWire( IDirect3DDevice9* device )
{
  device->SetMaterial( &Editor::Viewport::s_SelectedMaterial );
}
...
{
  device->SetMaterial( &Editor::Viewport::s_LiveMaterial );
}
...
bool Mesh::Pick( PickVisitor* pick )
{
  ...
  const Editor::Transform* t = GetTransform();
