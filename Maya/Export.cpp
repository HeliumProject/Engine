...
namespace Maya
{
                ...
                const MObjectArray              & nodes )
  {
    ...
    std::vector< i32 > exportInfoIndex;
    result.clear();
  ...
  void GetExportInfo( V_ExportInfo & result,
                const MObject      & node,
                      ...
                      int            index )
  {
    std::vector< i32 > result_index;
    GetExportInfo( result_index, node, artNumber, type, index );
    std::vector< i32 >::iterator itr = result_index.begin();
    std::vector< i32 >::iterator end = result_index.end();
    result.clear();
    for ( ; itr != end ; ++itr )
    {
  ...
  void GetExportInfo( std::vector< i32 > & result,
                const MObject          & node,
                      int                artNumber,
                      ContentType        type,
                      int                index )
  ...
  bool MAYA_API DescendantOf( const MObject& node, const Content::ContentType type, bool recurse )
  {
    if( node.isNull() ) 
    {
      return false;
  ...
  bool MAYA_API DescendantOf( const MObject& node, const MTypeId& typeID, bool recurse )
  {
    if( node.isNull() ) 
    {
      return false;
