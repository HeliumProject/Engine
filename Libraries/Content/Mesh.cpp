...
int Mesh::GetExportTypeIndex( Content::ContentType exportType ) const
{
  M_ContentTypeToIndex::const_iterator findItor = m_ExportTypeIndex.find( exportType );
  if( findItor != m_ExportTypeIndex.end() )
  {
...
bool Mesh::LightmapUVsInRange() const
{
  V_Vector2::const_iterator itor = m_LightMapUVs.begin();
  V_Vector2::const_iterator end = m_LightMapUVs.end();
  for ( ; itor != end; ++itor )
  {
    const Vector2& uv = *itor;
    if( uv.x > 1.0f || uv.x < 0.0f || uv.y > 1.0f || uv.y < 0.0f )
    {
      Console::Debug( "Bad UV: %f %f", uv.x, uv.y );
      return false;
    }
  }
  return true;
...
bool Mesh::IsRenderableType() const
{
  return m_ExportTypes[ Content::ContentTypes::Geometry ]
      || m_ExportTypes[ Content::ContentTypes::Bangle ]
      || m_ExportTypes[ Content::ContentTypes::Overlay ]
      || m_ExportTypes[ Content::ContentTypes::PreShell ]
      || m_ExportTypes[ Content::ContentTypes::BloomPreShell ]
...
f32 Mesh::SurfaceArea( Math::Scale* scale ) const
{
  ...
  if( !scale )
  {
    V_u32::const_iterator itor = m_TriangleVertexIndices.begin();
    V_u32::const_iterator end  = m_TriangleVertexIndices.end();
    for( ; itor != end; itor+=3 )
    {
...
f32 Mesh::UVSurfaceArea( UVSetType uvSetType ) const
{
  const V_Vector2* uvSet = NULL;

  if( uvSetType == UVSetTypes::Lightmap )
    uvSet = &m_LightMapUVs;
  else if( uvSetType == UVSetTypes::Base )
    uvSet = &m_BaseUVs;
  else if( uvSetType == UVSetTypes::Blend )
    uvSet = &m_BlendMapUVs;

  f32 area = 0.0f;
  if(uvSet && !uvSet->empty() )
  {
    NOC_ASSERT( m_TriangleVertexIndices.size()%3 == 0 );
    ...
    for( ; itor != end; itor+=3 )
    {
      Vector2 edge1 = (*uvSet)[*(itor+1)] - (*uvSet)[*itor];
      Vector2 edge3 = (*uvSet)[*(itor+2)] - (*uvSet)[*itor];

      f32 dot = edge1.Dot( edge3 );
      f32 tmp = sqrt( ( edge1.LengthSquared()*edge3.LengthSquared() ) -  ( dot*dot ) ) * 0.5f;
      if( !_isnan( tmp ) )
        area += tmp;
    }
  }
...
f32 Mesh::LightmapTexelsPerMeter( u32 logTextureSize, Math::Scale* scale ) const
{
  u32 dim = 1 << logTextureSize;

  f32 uvArea = UVSurfaceArea( UVSetTypes::Lightmap );
...
void Mesh::WeldNavMeshVerts(const f32 vertex_merge_threshold)
{
  ...
  //fix tri data
  V_u32::iterator itor = m_TriangleVertexIndices.begin();
  V_u32::iterator end  = m_TriangleVertexIndices.end();
  for( ; itor != end; ++itor )
  {
...
{
  V_u32::const_iterator iter =  m_WireframeVertexIndices.begin();
  u32 edge_id = 0;
...
void Mesh::NopTrisByTriList(const V_u32& ip_tris)
{
  for (V_u32::const_iterator it=ip_tris.begin(); it!=ip_tris.end(); ++it)
  {
...
void Mesh::NopTrisByVertList(const V_u32& ip_verts)
{
  ...
  for (u32 i=0; i<num_tris; ++i)
  {
    ...
    V_u32::const_iterator it = std::find(ip_verts.begin(), ip_verts.end(), tr_vert_ids[0]);
    if (it != ip_verts.end())
    {
...
void Mesh::NopTrisByEdgeList( const V_u32& ip_edges )
{
  V_u32 tris;
  u32 num_tris = (u32)(m_TriangleVertexIndices.size()/3);

  for (V_u32::const_iterator it=ip_edges.begin(); it!=ip_edges.end(); it++)
  {
...
{
  V_u8 vert_is_in_tris;
  vert_is_in_tris.resize(m_Positions.size(), 0);
  for (V_u32::const_iterator it=m_TriangleVertexIndices.begin(); it!=m_TriangleVertexIndices.end(); ++it)
  {
    if ((*it) != 0xFFFFFFFF)
    {
  ...
  u32 vert_id = 0;
  for (V_u8::const_iterator it=vert_is_in_tris.begin(); it!=vert_is_in_tris.end(); ++it, ++vert_id)
  {
    if ((*it))
    {
  ...
  V_u32 new_tri_vert_ids;
  V_u32 new_edge_vert_ids;
...
void Mesh::DeleteTris(const V_u32& ip_tris)
{
  NopTrisByTriList(ip_tris);
  PruneVertsNotInTris();
}
void Mesh::DeleteVerts(const V_u32& ip_verts)
{
  NopTrisByVertList(ip_verts);
  PruneVertsNotInTris();
}

void Mesh::DeleteEdges( const V_u32& ip_edges )
{
...
u32 Mesh::GetClosestTri(const Math::Vector3& sphere_start_pos, const f32& sphere_rad, const Math::Vector3& swept_dir, const f32& len)
{
  ...
  V_u32::const_iterator iter =  m_TriangleVertexIndices.begin();
  u32 tri_id = 0;
...
void Mesh::MergeVertToClosest(u32 ip_vert_id)
{
  ...
  V_u32 tris_to_be_deleted;//ones with both vert_id and closest_vert_id
  {
    u32 tri_id = 0;
    V_u32 invalid_tris;
    V_u32::const_iterator iter =  m_TriangleVertexIndices.begin();
